...
const uaParsers = {
    win: function(userAgent) {
        ...
        let matches = userAgent.match(/windows phone (\d+).(\d+)/i) || userAgent.match(/windows nt (\d+).(\d+)/i);
        const version = [];
...
const Devices = Class.inherit({
    ...
    */
    ...
    _fromUA: function(ua) {
        ...
        const isMac = /(mac os)/.test(ua.toLowerCase());
        const deviceWithOS = DEFAULT_DEVICE;
