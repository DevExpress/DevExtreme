...
describe('View', () => {
  it('render', () => {
    ...
    const tree = shallow(
      (<PagesLargeComponent {...({ pages } as any)} />) as any,
    );
...
describe('Pager pages logic', () => {
  ...
  it('pages: pageIndex: 4', () => {
    ...
    expect(pages.pages[0].pageProps).toMatchObject({
      index: 0,
      selected: false,
    });
    expect(pages.pages[0].key).toEqual('0');
    expect(pages.pages[1].key).toEqual('low');
    expect(pages.pages[3].pageProps).toMatchObject({
      index: 4,
      selected: true,
    });
  ...
  it('pages: rtlEnabled: true', () => {
    ...
    expect(pages.pages[2].pageProps).toMatchObject({
      index: 6,
      selected: false,
    });
    expect(pages.pages[3].pageProps).toMatchObject({
    ...
    expect(pages.pages[4].pageProps).toMatchObject({
      index: 4,
      selected: true,
    });
    expect(pages.pages[7].pageProps).toMatchObject({
      index: 0,
      selected: false,
    });
    expect(pages.pages).toHaveLength(8);
