      ...
      each([true, false]).describe('isSwipeDownStrategy: %o', (isSwipeDownStrategy) => {
        each([true, false]).describe('pullDownEnabled: %o', (pullDownEnabled) => {
          each([TopPocketState.STATE_RELEASED, TopPocketState.STATE_READY,
            ...
            each([50, 100]).describe('initPageY: %o', (initPageY) => {
              each([25, 75, 175, 200]).describe('pageY: %o', (pageY) => {
                it('handleMove(), locked: false, tryGetAllowedDirection: vertical', () => {
                  ...
                  if (forceGeneratePockets && isSwipeDownStrategy) {
                    ...
                    || pocketState === TopPocketState.STATE_PULLED) {
                      ...
                      if (top < height) {
                        expectedIsFakeRefreshState = true;
                      }
            ...
            each([jest.fn(), undefined]).describe('onPullDown: %o', (onPullDown) => {
              afterEach(() => {
                jest.clearAllMocks();
              });

              each([true, false]).describe('isPullDown: %o', (isPullDown) => {
                it('handleEnd(), should call onPullDown', () => {
                  const topPocketClientHeight = 40;
                  const scrollableOffsetHeight = 200;
                  const e = {
                    ...defaultEvent,
                  ...
                  expect(viewModel.topPocketState).toEqual(expectedTopPocketState);
                  expect(viewModel.pullDownOpacity).toEqual(expectedPullDownOpacity);
                  expect(viewModel.isFakeRefreshState).toEqual(expectedIsFakeRefreshState);
  ...
  describe('Getters', () => {
    each([DIRECTION_VERTICAL, DIRECTION_HORIZONTAL, DIRECTION_BOTH]).describe('Direction: %o', (direction) => {
      ...
      each([TopPocketState.STATE_RELEASED, TopPocketState.STATE_READY,
        ...
        TopPocketState.STATE_TOUCHED, TopPocketState.STATE_REFRESHING]).describe('pocketState: %o', (pocketState) => {
        each([true, false]).describe('PullDownEnabled: %o', (pullDownEnabled) => {
          each([-10, 30, 69, 70, 100]).describe('DeltaY: %o', (deltaY) => {
            it('isPullDown()', () => {
              const scrollableRef = {
                current: {
                  offsetHeight: 200,
      ...
      each(['swipeDown', 'pullDown']).describe('RefreshStrategy: %o', (refreshStrategy) => {
        it('isSwipeDownStrategy()', () => {
          const viewModel = new Scrollable({ direction });
