...
const _option = DOMComponentWithTemplate.prototype.option;
...
const getEmptyComponent = function() {
    ...
    const EmptyComponent = DOMComponentWithTemplate.inherit(emptyComponentConfig);
...
module.exports = isServerSide ? getEmptyComponent() : DOMComponentWithTemplate.inherit({
    ...
    _calculateCanvas: function() {
        ...
        const elementWidth = !sizeIsValid(size.width) && windowUtils.hasWindow() ? _floor(that._$element.width()) : 0;
        const elementHeight = !sizeIsValid(size.height) && windowUtils.hasWindow() ? _floor(that._$element.height()) : 0;
        let canvas = {
