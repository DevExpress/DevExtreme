...
var SchedulerAppointments = CollectionWidget.inherit({
    ...
    _repaintAppointments: function(appointments) {
        ...
        for(var i = 0; i < itemsLength; i++) {
            ...
            if(itemData && item.needRepaint === false) {
                this._resetItemSortedIndex(item);
            }
    ...
    _resetItemSortedIndex: function(item) {
        ...
        $.each($items, function(index, $item) {
            $item.dxSchedulerAppointment("instance").option("sortedIndex", item.settings[index].sortedIndex);
        });
    },
    ...
    _renderItem: function(index, item) {
        ...
        for(var i = 0; i < coordinateCount; i++) {
            ...
            this.callBase(index, item.itemData, $container);
        }
    },
    ...
    _getDateRange: function(e, startDate, endDate) {
        ...
        if(renderingStrategy !== "vertical" || this.invoke("isAllDay", itemData)) {
        ...
        if(cond) {
            startTime = renderingStrategy !== "horizontalMonth" ? this._correctStartDateByDelta(startDate, deltaTime) : startDate.getTime() - deltaTime;
            endTime = endDate.getTime();
        } else {
            startTime = startDate.getTime();
            endTime = renderingStrategy !== "horizontalMonth" ? this._correctEndDateByDelta(endDate, deltaTime) : endDate.getTime() + deltaTime;
        }
