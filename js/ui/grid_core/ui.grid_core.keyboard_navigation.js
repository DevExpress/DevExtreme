    ...
    focus: function(element) {
        ...
        if(!element) {
            if(focusedRowEnabled) {
                activeElementSelector = ".dx-row[tabindex]";
            } else {
                activeElementSelector = ".dx-group-row[tabIndex], .dx-row > td[tabindex]";
            }
            element = this.component.$element().find(activeElementSelector).first();
        }

        if(element) {
            this._focusElementInActiveView($(element), false);
        }
    },

    _focusElementInActiveView: function($element, isHighlighted) {
