...
const KeyboardNavigationController = core.ViewController.inherit({
    ...
    _clickHandler: function(e) {
        ...
        const focusedViewElement = data.view && data.view.element();
        ...
        each(that._focusedViews, function(index, view) {
            if(view) {
                view.renderCompleted.add(function(e) {
                    const $rowsView = view.element();
                    const isFullUpdate = !e || e.changeType === 'refresh';
                    const isFocusedViewCorrect = that._focusedView && that._focusedView.name === view.name;
                    let needUpdateFocus = false;
                    const isAppend = e && (e.changeType === 'append' || e.changeType === 'prepend');
                    const keyboardActionSelector = `.${ROW_CLASS} > td, .${ROW_CLASS}`;
    ...
    _scrollToElement: function($element, offset) {
        const scrollable = this._focusedView.getScrollable();
    ...
    */
    ...
    _focusElement: function($element, isHighlighted) {
        const focusView = this._getFocusedViewByElement($element);
        const isRowFocusType = this.isRowFocusType();
        let args = { };
    ...
    focusViewByName: function(viewName) {
        const view = this._getFocusedViewByName(viewName);
        ...
        views: {
            rowsView: {
                ...
                updateFocusElementTabIndex: function(cellElements) {
                    ...
                    const tabIndex = that.option('tabIndex');
