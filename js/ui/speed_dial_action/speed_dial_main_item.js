...
class SpeedDialMainItem extends SpeedDialItem {
    ...
    _getCurrentOptions(actions) {
        ...
        return visibleActions.length === 1 ?
            extend(visibleActions[0]._optionSilent(), { position: this._getPosition() }) :
    ...
    _renderActions() {
        ...
        for(let i = 0; i < actions.length; i++) {
            ...
            action._optionSilent("actionComponent", action);
            action._optionSilent("parentPosition", this._getPosition());
            action._optionSilent("actionVisible", action._optionSilent("visible"));

            this._actionItems.push(this._createComponent($actionElement, SpeedDialItem, extend({}, action._optionSilent(), { visible: false })));
        }
    }
    ...
    _getActionAnimation(action, index, lastActionIndex) {
        ...
        action._optionSilent("animation.show.delay", actionAnimationDelay * index);
        action._optionSilent("animation.hide.delay", actionAnimationDelay * (lastActionIndex - index));

        return action._optionSilent("animation");
    }
    ...
    _getActionPosition(actions, index) {
        ...
        const actionOffsetX = action._optionSilent("label") && !this._$label ?
        ...
        const actionPositionAtMy = action._optionSilent("label") ?
...
exports.initAction = function(newAction) {
    ...
    delete newAction._optionSilent("onInitializing");
    ...
    if(!speedDialMainItem) {
        ...
        speedDialMainItem = newAction._createComponent($fabMainElement, SpeedDialMainItem,
            extend({}, newAction._optionSilent(), {
                actions: [ newAction ]
            })
        );
    } else {
        ...
        savedActions.forEach(action => {
            if(action._optionSilent("id") === newAction._optionSilent("id")) {
                isActionExist = true;
                return newAction;
            }
        });

        delete speedDialMainItem._optionSilent("position");
        ...
        if(!isActionExist) {
            ...
            speedDialMainItem.option(extend({}, savedActions[0]._optionSilent(), {
                actions: savedActions,
...
exports.disposeAction = function(actionId) {
    ...
    savedActions = savedActions.filter(action => {
        return action._optionSilent("id") !== actionId;
    });
