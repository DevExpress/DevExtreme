...
const PivotGrid = Widget.inherit({
    ...
    _renderFieldChooser() {
        ...
        if (that._fieldChooserPopup) {
            ...
            that._fieldChooserPopup = that._createComponent($(DIV)
                .addClass(FIELD_CHOOSER_POPUP_CLASS)
                .appendTo(container), Popup, popupOptions);
        }
    },
    _renderContextMenu() {
        const that = this;
    ...
    updateDimensions() {
        ...
        const calculateGroupHeight = (dataAreaHeight, totalHeight, hasRowsScroll, hasColumnsScroll, scrollBarWidth) => (hasRowsScroll
            ? dataAreaHeight
        ...
        deferUpdate(() => {
            ...
            if (that._hasHeight) {
                ...
                dataAreaHeight = getHeight(that.$element()) - filterAreaHeight - dataHeaderHeight
                    - (Math.max(getHeight(that._dataArea.headElement()), getHeight(columnAreaCell), descriptionCellHeight)
                        + bordersWidth);
            }
