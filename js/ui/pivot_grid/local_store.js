        ...
        $.each(fields || [], function(_, field) {
            ...
            if(!field.selector) {
                ...
                if(!dataField) {
                    ...
                    dataSelector = dataField.indexOf(".") !== -1 ? dataCoreUtils.compileGetter(dataField) : function(data) { return data[dataField]; };
                }
    ...
    function loadDataSource(dataSource, reload) {
        ...
        if(!dataSource.isLoaded() || reload) {
            ...
            when(loadDeferred).always(function() {
                loadDataSource(dataSource).done(function() {
                    d.resolve(filterDataSource(dataSource));
                }).fail(d.reject);
            });
        } else {
            d.resolve(filterDataSource(dataSource));
