...
export const XmlaStore = Class.inherit((function () {
    ...
    function generateAxisMdx(options, axisName, cells, withArray, parseOptions) {
        ...
        if (crossJoins.length) {
            ...
            if (axisName === 'rows' && options.rowTake) {
                expression = stringFormat(mdxSubset, expression, options.rowSkip > 0
                    ? options.rowSkip + 1
                    : 0, options.rowSkip > 0
                    ? options.rowTake
                    : options.rowTake + 1);
            ...
            if (axisName === 'columns' && options.columnTake) {
                expression = stringFormat(mdxSubset, expression, options.columnSkip > 0
    ...
    return {
        ...
        load(options) {
            ...
            const load = () => {
                if (mdxString) {
                    when(sendQuery(storeOptions, mdxString), rowCountMdx && sendQuery(storeOptions, rowCountMdx))
