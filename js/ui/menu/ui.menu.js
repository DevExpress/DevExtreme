...
var Menu = MenuBase.inherit({
    ...
    _toggleAdaptiveMode: function(state) {
        var $menuItemsContainer = this.element().find("." + DX_MENU_ITEMS_CONTAINER_CLASS).parent(),
    ...
    _getTreeViewOptions: function() {
        ...
        return $.extend(menuOptions, {
            ...
            onItemExpanded: $.proxy(function(e) { this._actions["onSubmenuShown"](e); }, that),
            onItemCollapsed: $.proxy(function(e) { this._actions["onSubmenuHidden"](e); }, that),
    ...
    _attachSubmenuHandlers: function($rootItem, submenu) {
        ...
        $.each(submenus, function(index, submenu) {
            $(submenu)
                ...
                .on(submenuMouseLeaveName, null, $.proxy(that._submenuMouseLeaveHandler, that, $rootItem));
        });
    },
    ...
    _hoverStartHandler: function(e) {
        ...
        if(!this._hasChildren(node)) {
            this._showSubmenuTimer = setTimeout($.proxy(this._hideSubmenuAfterTimeout, this), this._getDelay("hide"));
            return;
        }
