    ...
    _attachFocusOutHandler: function() {
        if(isIOs) {
            ...
            eventsEngine.on(this._inputWrapper(), eventUtils.addNamespace("focusout", this.NAME), function(event) {
                ...
                var popupContent = this.content ? $(this.content()).parent().parent() : this._$popup;
                if(newTarget && this.option("opened")) {
                    var isNewTargetOutside = $(newTarget).closest("." + DROP_DOWN_EDITOR_OVERLAY, popupContent).length === 0;
