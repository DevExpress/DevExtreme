...
const Scrollable = DOMComponent.inherit({
    ...
    scrollTo: function(targetLocation) {
        ...
        if(this._isScrollInverted()) {
            ...
            if(positive) {
                // support webkit < v.86
                location.left = Math.abs(location.left - this._getMaxOffset().left);
            } else {
    ...
    getScrollElementPosition: function($element, direction, offset) {
        ...
        const elementPosition = ((this._isRtlNativeStrategy() && !isVertical) ? this._getMaxOffset().left + elementPositionRelativeToContent : elementPositionRelativeToContent);
