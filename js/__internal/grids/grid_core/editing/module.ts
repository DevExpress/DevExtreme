...
const EditingController = modules.ViewController.inherit((function () {
  ...
  function getButtonName(button) {
    return isObject(button) ? (button as any).name : button;
  }
        ...
        items.some((item, index) => {
          ...
          if (isObject(item)) {
            if (isProcessedItem || isDefined(item[INSERT_INDEX])) {
              if (equalByValue((item as any).key, key)) {
                result = index;
              }
      ...
      if (button.template && useLegacyColumnButtonTemplate) {
        ...
        if (button.template) {
          ...
          if (iconType === 'image' || iconType === 'svg') {
            $button = (iconUtils as any).getImageContainer(icon).addClass(button.cssClass);
          } else {
