...
class BingProvider extends DynamicProvider {
  ...
  updateBounds() {
    return Promise.all([
      ...
      const bounds = Microsoft.Maps.LocationRect.fromLocations(result[0], result[1]);
  ...
  _renderRoute(options) {
    return Promise.all(map(options.locations, (point) => this._resolveLocation(point))).then((locations) => new Promise((resolve) => {
      ...
      const routeColor = Microsoft.Maps.Color.fromHex(color);
  ...
  _extendBounds(location) {
    if (this._bounds) {
      this._bounds = Microsoft.Maps.LocationRect.fromLocations(this._bounds.getNorthwest(), this._bounds.getSoutheast(), location);
    } else {
...
/// #DEBUG
(BingProvider as any).remapConstant = function (newValue) {
