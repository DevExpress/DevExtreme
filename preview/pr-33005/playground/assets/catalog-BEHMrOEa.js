import { j as jQuery, _ as __vitePreload, s as setLicenseCheckSkipCondition } from './preload-helper-gWX0XG96.js';
...
function loadDemo(widget, name) {
  ...
  if (entry?.files.length) {
    ...
    const $bar = jQuery('<div class="demo-files-bar">').appendTo($container);
    jQuery('<span class="demo-files-label">').text('Source:').appendTo($bar);
    entry.files.forEach(file => {
      jQuery('<a class="demo-file-link">').attr('href', `vscode://file/${jqueryDir}/${file}`).text(file).appendTo($bar);
    });
  }
