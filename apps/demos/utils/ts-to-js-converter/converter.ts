...
const formatOutputs = async (
  ...
  // index.html is excluded: it is generated by the demo builder (utils/build/), not
  // authored here, and the forced flags below would reflow it away from the builder's
  // output — leaving `lint-html` and every in-place rebuild fighting each other.
  const prettierPatterns = outDirs
    .map((outDir) => quoteShellArg(`${path.resolve(outDir)}${path.sep}!(*.{css,json,md,tsbuildinfo,html})`))
    .join(' ');
