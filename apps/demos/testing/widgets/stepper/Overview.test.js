...
runManualTest('Stepper', 'Overview', (test) => {
  test('Stepper Overview Appearance', async (t) => {
    ...
    await t
      .click($(`#${ORIENTATION_BUTTON_GROUP_ID}`).withText('Vertical'));

    await testScreenshot(t, takeScreenshot, 'Stepper overview rtl=true, orientation=vertical.png');

    await t
      .click($(`#${RTL_MODE_CHECK_BOX_ID}`))
      .click($(`#${ORIENTATION_BUTTON_GROUP_ID}`).withText('Horizontal'));

    const steppers = await $(`.${STEP_LIST_CLASS}`);

    await t
      .click($(`#${NAVIGATION_MODE_BUTTON_GROUP_ID}`).withText('Linear'));
    ...
    await t
      .click($(`#${NAVIGATION_MODE_BUTTON_GROUP_ID}`).withText('Non-linear'));
