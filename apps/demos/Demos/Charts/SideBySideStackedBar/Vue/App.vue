...
function customizeItems(items: Array<LegendItem>): Array<LegendItem> {
  const sortedItems: Array<LegendItem> = [];

  items.forEach((item) => {
    const startIndex = (item.series as Record<string, any>)?.stack === 'male' ? 0 : 3;
