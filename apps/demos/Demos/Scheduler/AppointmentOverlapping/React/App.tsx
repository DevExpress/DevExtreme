...
import Scheduler, { type SchedulerRef, type SchedulerTypes } from 'devextreme-react/scheduler';
...
const App = () => {
  ...
  const detectConflict = useCallback((newAppt: Appointment): boolean => {
    const scheduler = schedulerRef.current?.instance?.();
    if (!scheduler) return false;

    const allItems = scheduler.getDataSource().items() as Appointment[];
  ...
  const onAllowOverlappingChanged = useCallback((e: SwitchTypes.ValueChangedEvent) => {
    setAllowOverlapping(e.value);
  }, []);

  const onAppointmentFormOpening = useCallback((e: SchedulerTypes.AppointmentFormOpeningEvent) => {
    const { form } = e;
