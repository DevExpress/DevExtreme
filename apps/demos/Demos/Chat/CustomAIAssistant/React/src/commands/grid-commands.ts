...
To filter a date column by a year and/or month, use operator "anyof" with value as an array of one or more strings in "YYYY" or "YYYY/M" format. Only use "anyof" when the year is known; if the year is missing, omit the action instead of guessing.`,
    ...
    execute(grid, args, rawText) {
      ...
      try {
        const newCondition: FilterCondition = [columnName, args.operator ?? '=', value as FilterCondition[2]];
...
export function getFilterConditions(filterValue: unknown): FilterCondition[] {
  ...
  return Array.isArray(filterValue[0])
    ? filterValue.filter(Array.isArray) as FilterCondition[]
...
export function getGridColumnNames(grid: TaskGrid): string[] {
  return (grid.option('columns') as DataGridTypes.Column[]).map((column) => String(column.dataField));
}

export function applyGridActions(grid: TaskGrid, actions: ExecuteGridAssistantAction[], rawText?: string): CommandResult[] {
