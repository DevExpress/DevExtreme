The DevExtreme DataGrid supports data paging. The component can load rows in portions to improve performance when working with large data sets. This demo allows you to navigate between pages using the DataGrid's built-in pager.
<!--split-->
...
In this demo, you can use controls below the DataGrid to change the pager display mode and toggle the visibility of individual pager elements. Note that in compact mode, navigation buttons are always visible.
