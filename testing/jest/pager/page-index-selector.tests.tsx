...
jest.mock('../../../js/renovation/pager/pages-small.p', () => { return () => { return null; }; });
jest.mock('../../../js/renovation/pager/pages-large.p', () => { return () => { return null; }; });
...
describe('Page index selector', () => {
    ...
    it('render showNavigationButtons: false, isLargeDisplayMode:true', () => {
        const { root, pages } = render({ isLargeDisplayMode: true, showNavigationButtons: false, pageIndex: 6, pageCount: 16, maxPagesCount: 4 });
        expect(root.find(LightButton)).toHaveLength(0);
        expect(pages().props()).toMatchObject({ pageCount: 16, pageIndex: 6, rtlEnabled: false, maxPagesCount: 4 });
    });
    it('render showNavigationButtons: true, isLargeDisplayMode:false', () => {
        const { root, pages } = render({ isLargeDisplayMode: false, showNavigationButtons: true, pageIndex: 5, pageCount: 10, pagesCountText: 'Off' });
        expect(root.find(LightButton)).toHaveLength(2);
        expect(pages().props()).toMatchObject({ pageCount: 10, pageIndex: 5, rtlEnabled: false, pagesCountText: 'Off' });
    });
