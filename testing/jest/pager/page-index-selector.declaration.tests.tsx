...
describe('Page index selector', () => {
    const createViewModel = (props: Props) => {
        return new PageIndexSelector(props);
    };
    it('render showNavigationButtons: true, isLargeDisplayMode:true', () => {
        const { nextClassName, prevClassName } = createViewModel({ isLargeDisplayMode: true, showNavigationButtons: true, pageIndex: 5, pageCount: 10 });
    ...
    it('render showNavigationButtons: false, isLargeDisplayMode:true', () => {
        const { renderNavButtons } = createViewModel({ isLargeDisplayMode: true, showNavigationButtons: false, pageIndex: 5, pageCount: 10 });
        expect(renderNavButtons).toBeFalsy();
    });
    it('render showNavigationButtons: true, isLargeDisplayMode:false', () => {
        const { renderNavButtons } = createViewModel({ isLargeDisplayMode: false, showNavigationButtons: true, pageIndex: 5, pageCount: 10 });
    ...
    //     const { root, prevButton, nextButton } = createViewModel({ isLargeDisplayMode:true, showNavigationButtons: false, pageIndex: 5, pageCount: 10 });
    ...
    it('enable/disable navigation button rtl=false, firstPage', async () => {
        const { nextClassName, prevClassName } = createViewModel({ rtlEnabled: false, pageIndex: 0, pageCount: 10, isLargeDisplayMode: true, showNavigationButtons: true });
    ...
    it('enable/disable navigation button rtl=false, lastPage', async () => {
        const { nextClassName, prevClassName } = createViewModel({ rtlEnabled: false, pageIndex: 9, pageCount: 10, isLargeDisplayMode:true, showNavigationButtons: true });
        // root.update();
    ...
    it('enable/disable navigation button rtl=true, firstPage', async () => {
        const { nextClassName, prevClassName } = createViewModel({ rtlEnabled: true, pageIndex: 0, pageCount: 10, isLargeDisplayMode: true, showNavigationButtons: true });
        expect(prevClassName.indexOf(PAGER_BUTTON_DISABLE_CLASS)).toBe(-1);
        expect(nextClassName.indexOf(PAGER_BUTTON_DISABLE_CLASS)).not.toBe(-1);
    });
    it('enable/disable navigation button rtl=true, lastPage', async () => {
        const { nextClassName, prevClassName } = createViewModel({ rtlEnabled: true, pageIndex: 9, pageCount: 10, isLargeDisplayMode: true, showNavigationButtons: true });
        expect(prevClassName.indexOf(PAGER_BUTTON_DISABLE_CLASS)).not.toBe(-1);
        expect(nextClassName.indexOf(PAGER_BUTTON_DISABLE_CLASS)).toBe(-1);
    });
    it('click to navigation buttons pageIndex = 0', () => {
        const pageIndexChangeHandler = jest.fn();
        const viewModel = createViewModel({ rtlEnabled: false, pageIndex: 0, pageCount: 3, pageIndexChange: pageIndexChangeHandler, isLargeDisplayMode: true, showNavigationButtons: true });
    ...
    it('click to navigation buttons pageIndex = pageCount - 1', () => {
        const pageIndexChangeHandler = jest.fn();
        const viewModel = createViewModel({ rtlEnabled: false, pageIndex: 2, pageCount: 3, pageIndexChange: pageIndexChangeHandler, isLargeDisplayMode: true, showNavigationButtons: true });
        viewModel.navigateToNextPage();
        expect(pageIndexChangeHandler).not.toBeCalled();
        viewModel.navigateToPrevPage();
        expect(pageIndexChangeHandler).toBeCalled();
