...
namespace Runner.Controllers
{
    ...
    public class MainController : Controller
    {
        ...
        public void NotifySuiteFinalized(string name, bool passed)
        {
            ...
            lock (IO_SYNC)
            {
                ...
                if (_runFlags.IsContinuousIntegration)
                    IOFile.WriteAllText(Path.Combine(_env.ContentRootPath, "testing/qunit/LastSuiteTime.txt"), DateTime.Now.ToString("s"));
            }
        }
        ...
        {
            var xslUrl = Url.Content("~/testing/qunit/content/unittests.xsl");
        ...
        void LogMiscErrorCore(string data)
        {
            if (_runFlags.IsContinuousIntegration)
            {
                lock (IO_SYNC)
                {
                    IOFile.AppendAllText(Path.Combine(_env.ContentRootPath, "testing/qunit/MiscErrors.log"), data + Environment.NewLine);
                }
            }
        }
        ...
        {
            return Path.Combine(_env.ContentRootPath, "testing/qunit/Results.xml");
        }
