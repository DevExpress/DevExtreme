...
QUnit.test('Image with xlink:href', function(assert) {
    const that = this;
    const done = assert.async();
    const markup = testingMarkupStart + '<defs><clipPath id="clippath1"><rect x="0" y="30" width="500" height="30"></rect></clipPath></defs><image x="-10" y="-15" width="20" height="25" preserveAspectRatio="xMidYMid" transform="translate(427,82)" xlink:href="/testing/qunit/content/exporterTestsContent/test-image.png" visibility="visible" clip-path="url(#clippath1)"></image>' + testingMarkupEnd;
    const imageBlob = getData(markup);
    const context = window.CanvasRenderingContext2D.prototype;
    ...
    $.when(imageBlob).done(function(blob) {
        try {
            ...
            assert.deepEqual(that.drawnElements[2].args, {
                node: 'IMG',
                src: '/testing/qunit/content/exporterTestsContent/test-image.png',
                x: -10,
                y: -15,
                width: 20,
                height: 25
...
QUnit.test('Image with href', function(assert) {
    const that = this;
    const done = assert.async();
    const markup = testingMarkupStart + '<image x="-10" y="-15" width="20" height="25" preserveAspectRatio="xMidYMid" href="/testing/qunit/content/exporterTestsContent/test-image.png" visibility="visible"></image>' + testingMarkupEnd;
    const imageBlob = getData(markup);
...
QUnit.test('Export image in group', function(assert) {
    const done = assert.async();
    const markup = testingMarkupStart + '<g transform="translate(10, 10)"><image x="-10" y="-15" width="20" height="25" preserveAspectRatio="xMidYMid" href="/testing/qunit/content/exporterTestsContent/test-image.png" visibility="visible"></image></g>' + testingMarkupEnd;
    const imageBlob = getData(markup);
    const context = window.CanvasRenderingContext2D.prototype;
