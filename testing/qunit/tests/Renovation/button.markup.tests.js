...
QUnit.module('Button markup', () => {
    ...
    QUnit.test('icon as path', function(assert) {
        const element = $('#button').dxrButton({
            icon: '../../testing/qunit/content/add.png'
        });

        assert.ok(element.find('img[src=\'../../testing/qunit/content/add.png\']').length, 'icon was added by src');

        element.dxrButton('instance').option('icon', '../../testing/qunit/content/plus.png');
        assert.ok(element.find('img[src=\'../../testing/qunit/content/plus.png\']').length, 'icon was changed correctly');
    });
