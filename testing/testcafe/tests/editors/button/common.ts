...
test('Buttons render', async (t) => {
  ...
  await t.expect(await takeScreenshot(`buttons-render${getThemePostfix()}.png`, '#container')).ok();

  await insertStylesheetRule(`.${BUTTON_CLASS} { width: 70px }`, 0);

  await t.expect(await takeScreenshot(`buttons-render-with-overflow${getThemePostfix()}.png`, '#container')).ok();
  ...
  await t
    .expect(await takeScreenshot(`buttons-stretch-of-large-text${getThemePostfix()}.png`, '#container'))
    .ok()
