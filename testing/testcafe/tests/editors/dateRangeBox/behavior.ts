...
test('Value should be saved after select range in calendar and click on apply button, value: [null, null]', async (t) => {
  ...
  await t
    .click(dateRangeBox.getPopup().getApplyButton().element)
    .expect(dateRangeBox.option('value'))
    .eql([new Date(2021, 9, 6), new Date(2021, 9, 17)])
    .expect(ClientFunction(() => (window as any).onValueChangedCounter)())
    .eql(1)
...
test('Value should not be saved after select range and click on cancel button', async (t) => {
  const dateRangeBox = new DateRangeBox('#container');
  ...
  await t
    .click(dateRangeBox.getPopup().getCancelButton().element)
    .expect(dateRangeBox.option('opened'))
    .eql(false)
    .expect(dateRangeBox.option('value'))
    .eql([null, null])
...
test('Open by click on startDate input and reselect start date in calendar, value: [null, null]', async (t) => {
  const dateRangeBox = new DateRangeBox('#container');
