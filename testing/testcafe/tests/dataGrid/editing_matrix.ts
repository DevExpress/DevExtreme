...
EDITING_MODES.forEach((mode) => {
  const isFormMode = mode === 'form' || mode === 'popup';
      ...
      useMaskValues.forEach((useMask) => {
        ...
        useKeyboardValues.forEach((useKeyboard) => {
          repaintChangesOnlyValues.forEach((repaintChangesOnly) => {
            ...
            const createDataGrid = (): Promise<void> => createWidget('dxDataGrid', {
              keyExpr: 'id',
              dataSource: [
                {
            ...
            const editCell = async (
              t: TestController, rowIndex: number, modifyFirstColumn = false,
            ...
            const checkEditCell = async (
              t: TestController, cell: DataCell, editor: Selector,
            ): Promise<void> => {
              if (!isFormMode) {
            ...
            const getEditorValue = async (editor: Selector): Promise<string> => {
            ...
            const getCellText = (cell: DataCell): Promise<string> => {
              if (dataField === 'boolean') {
                return getEditorValue(cell.getEditor().element);
              }
            ...
            const checkModifiedCell = async (
              t: TestController, cell: DataCell, editor: Selector, value: string,
            ): Promise<void> => {
              let editorText;
              if (mode === 'batch' || mode === 'cell') {
                editorText = await getCellText(cell);
              } else {
                editorText = await getEditorValue(editor);
              }
            ...
            const checkSavedCell = async (
              t: TestController, cell: DataCell, value: string,
            ): Promise<void> => {
              await t
                .expect(await getCellText(cell))
                .eql(value);
            ...
            const clickSaveButton = async (t: TestController, rowIndex: number): Promise<void> => {
              const form = getEditForm();
            ...
            const setEditorValue = async (
              t: TestController, editor: Selector, value: string,
            ): Promise<void> => {
            ...
            const editNextCell = async (
              t: TestController, rowIndex: number,
              ...
              await checkEditCell(t, nextCell, nextEditor);
            ...
            test(`Update cell value ${JSON.stringify({
              ...
              const { cell, editor } = isAdding
                ? await addRow(t)
                : await editCell(t, rowIndex, true);

              await checkEditCell(t, cell, editor);

              await setEditorValue(t, editor, useMask ? newMaskValue : newValue);

              await clickSaveButton(t, isAdding ? rowIndex : 0);
              await checkSavedCell(t,
            ...
            if (isBasicColumn && !isAdding) {
              test(`Edit next cell ${JSON.stringify({
                ...
                const { cell, editor } = await editCell(t, rowIndex);
                await checkEditCell(t, cell, editor);

                await setEditorValue(t, editor, useMask ? newMaskValue : newValue);
                const { nextCell, nextEditor } = await editNextCell(t, rowIndex);
                await checkEditCell(t, nextCell, nextEditor);
                await checkModifiedCell(t, cell, editor, newValue);
              }).before(createDataGrid);
            }
          });
        });
