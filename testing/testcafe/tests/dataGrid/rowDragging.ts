...
const moveRow = function(grid: any, rowIndex: number, x: number, y: number): Promise<void> {
    ...
    { dependencies: { grid, rowIndex, x, y } },
    )();
}

const isPlaceholderVisible = function(grid: any): Promise<boolean> {
    return ClientFunction(() => {
        const gridInstance = grid.getGridInstance() as any;
        const sortable = gridInstance.getView('rowsView')._sortable;

        return $(sortable._$placeholderElement).is(':visible');
    },
    { dependencies: { grid } },
    )();
}
...
test('The placeholder should appear when a cross-component dragging rows after scrolling the window', async (t) => {
    ...
    await t.expect(isPlaceholderVisible(treeList)).ok();
  }).before(async (t) => {
    let result;
    ...
    createWidget('dxDataGrid', {
        ...
        result = createWidget('dxTreeList', {
