...
test('it should correctly change allDayPanelOption at runtime', async (t, { platform }) => {
  const scheduler = new Scheduler('#container');
  ...
  await updateComponentOptions(platform, { allDayPanelMode: 'allDay' });
  await t
    .expect(scheduler.getAppointmentCount())
    .eql(5);
  await checkAllDayAppointment(t, scheduler, 'allDay', 0, undefined, 95);
  ...
  await updateComponentOptions(platform, { allDayPanelMode: 'hidden' });
  await t
    .expect(scheduler.getAppointmentCount())
    .eql(5)
