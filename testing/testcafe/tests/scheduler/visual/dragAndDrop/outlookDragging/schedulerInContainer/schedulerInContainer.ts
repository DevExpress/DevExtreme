...
test('Dragging should be work right in case dxScheduler placed in dxTabPanel', async (t) => {
  ...
  await t
    ...
    .expect(await takeScreenshot('placed-in-dxTabPanel-drag-to-bottom.png'))
    .ok()

    .drag(draggableAppointment.element, 0, -170, { speed: 0.1 })
    .expect(await takeScreenshot('placed-in-dxTabPanel-drag-to-top.png'))
    .ok()

    .drag(draggableAppointment.element, 100, 0, { speed: 0.1 })
    .expect(await takeScreenshot('placed-in-dxTabPanel-drag-to-right.png'))
    .ok()
