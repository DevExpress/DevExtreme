...
test('Dragging should be work right in case dxScheduler placed in container with transform style', async (t) => {
  ...
  await t
    ...
    .expect(await takeScreenshot('placed-in-transform-container-drag-to-bottom.png'))
    .ok()

    .drag(draggableAppointment.element, 0, -170, { speed: 0.1 })
    .expect(await takeScreenshot('placed-in-transform-container-drag-to-top.png'))
    .ok()

    .drag(draggableAppointment.element, 100, 0, { speed: 0.1 })
    .expect(await takeScreenshot('placed-in-transform-container-drag-to-right.png'))
    .ok()

    .drag(draggableAppointment.element, -230, 0, { speed: 0.1 })
    .expect(await takeScreenshot('placed-in-transform-container-drag-to-left.png'))
    .ok()
