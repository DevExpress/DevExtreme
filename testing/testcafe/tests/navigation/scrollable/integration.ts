...
test('The rows in the fixed column are not aligned when the grid is encapsulated inside a <td> element, useNative: false (T1071725)', async (t) => {
  ...
  await t
    .expect(await takeScreenshot(`Grid with scrollable encapsulated inside td element, useNative=false${getThemePostfix()}.png`, Selector('#grid')))
    .ok()
...
test('The rows in the fixed column are not aligned when the grid is encapsulated inside a <td> element, useNative: true', async (t) => {
  ...
  await t
    .expect(await takeScreenshot(`Grid with scrollable encapsulated inside td element, useNative=true${getThemePostfix()}.png`, Selector('#grid')))
    .ok()
    .expect(compareResults.isValid())
    .ok(compareResults.errorMessages());
}).before(async () => {
