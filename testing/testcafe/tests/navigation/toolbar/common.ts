...
// const themes = ['generic.light', 'generic.dark', 'generic.light.compact', 'material.blue.light', 'material.blue.light.compact'];
...
['always', 'never'].forEach((locateInMenu) => {
  test(`Default nested widgets render,items[].locateInMenu=${locateInMenu}`, async (t) => {
    ...
    await t
      .expect(await takeScreenshot(`Default-nested-widgets-render,items[]locateInMenu=${locateInMenu}${getThemePostfix()}.png`, targetContainer))
      .ok()
  ...
  test(`Toolbar with dropDownButton,items[].locateInMenu=${locateInMenu}`, async (t) => {
    ...
    await t
      .expect(await takeScreenshot(`Toolbar-with-dropDownButton,items[]locateInMenu=${locateInMenu}${getThemePostfix()}.png`, targetContainer))
      .ok()
      .expect(compareResults.isValid())
      .ok(compareResults.errorMessages());
  }).before(async () => {
  ...
  ['text', 'outlined', 'contained'].forEach((stylingMode) => {
    test(`Toolbar with different types of buttons,items[{locateInMenu=${locateInMenu},stylingMode:${stylingMode}}]`, async (t) => {
      ...
      await t
        .expect(await takeScreenshot(`Toolbar-with-${stylingMode}-buttons,items[]locateInMenu=${locateInMenu}${getThemePostfix()}.png`, targetContainer))
        .ok()
        .expect(compareResults.isValid())
        .ok(compareResults.errorMessages());
    }).before(async () => {
...
test('Toolbar with buttonGroup', async (t) => {
  ...
  await t
    .expect(await takeScreenshot(`Toolbar_buttonGroup_appearence${getThemePostfix()}.png`))
    .ok()
    .expect(compareResults.isValid())
    .ok(compareResults.errorMessages());
  ...
  for (let i = 1; i < 6; i++) {
    ...
    await t
      .click(menuButton)
      .expect(await takeScreenshot(`Toolbar${i}_buttonGroup_openedMenuAppearence${getThemePostfix()}.png`))
      .ok()
      .click(menuButton);
  }
...
test('Default nested widgets render,items[].locateInMenu=auto', async (t) => {
  ...
  await t
    .expect(await takeScreenshot(`Default-nested-widgets-render,items[]locateInMenu=auto${getThemePostfix()}.png`, targetContainer))
    .ok()
    .expect(compareResults.isValid())
    .ok(compareResults.errorMessages());
}).before(async () => {
...
test('Toolbar with dropDownButton,items[].locateInMenu=auto', async (t) => {
  ...
  await t
    .expect(await takeScreenshot(`Toolbar-with-dropDownButton,items[]locateInMenu=always${getThemePostfix()}.png`, targetContainer))
    .ok()
    .expect(compareResults.isValid())
    .ok(compareResults.errorMessages());
}).before(async () => {
