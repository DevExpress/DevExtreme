...
['auto'].forEach((locateInMenu) => { // always
  ...
  states.forEach((state) => {
    test(`Toolbar buttons appearence${state ? `,${state.replaceAll('dx-state-', '')}` : ''},locateInMenu=${locateInMenu}`, async (t) => {
      ...
      await t
        .expect(await takeScreenshot(`Toolbar-buttons${state ? `,${state.replaceAll('dx-state-', '')}` : ''}${getThemePostfix()}.png`, targetContainer))
        .ok()
    ...
    ['template'].forEach((templateName) => { // 'menuItemTemplate'
      ...
      test(`Toolbar buttons as custom ${templateName} appearence${state ? `,${state.replaceAll('dx-state-', '')}` : ''},locateInMenu=${locateInMenu}`, async (t) => {
        ...
        await t
          .expect(await takeScreenshot(`Toolbar-buttons-custom-${templateName}${state ? `,${state.replaceAll('dx-state-', '')}` : ''}${getThemePostfix()}.png`, targetContainer))
          .ok()
          .expect(compareResults.isValid())
          .ok(compareResults.errorMessages());
      }).before(async () => {
    ...
    test(`Toolbar button group appearence${state ? `,${state.replaceAll('dx-state-', '')}` : ''},locateInMenu=${locateInMenu}`, async (t) => {
      ...
      await t
        .expect(await takeScreenshot(`Toolbar-buttonGroup${state ? `,${state.replaceAll('dx-state-', '')}` : ''}${getThemePostfix()}.png`, targetContainer))
        .ok()
        .expect(compareResults.isValid())
        .ok(compareResults.errorMessages());
    }).before(async () => {
    ...
    ['template'].forEach((templateName) => { // 'menuItemTemplate'
      ...
      test(`Toolbar button group as custom ${templateName} appearence${state ? `,${state.replaceAll('dx-state-', '')}` : ''},locateInMenu=${locateInMenu}`, async (t) => {
        ...
        await t
          .expect(await takeScreenshot(`Toolbar-buttonGroup-custom-${templateName}${state ? `,${state.replaceAll('dx-state-', '')}` : ''}${getThemePostfix()}.png`, targetContainer))
          .ok()
          .expect(compareResults.isValid())
          .ok(compareResults.errorMessages());
      }).before(async () => {
  ...
  test(`Toolbar all widgets appearance,locateInMenu=${locateInMenu}`, async (t) => {
    ...
    await t
      .expect(await takeScreenshot(`Toolbar all widgets appearance${getThemePostfix()}.png`, targetContainer))
      .ok()
      .expect(compareResults.isValid())
      .ok(compareResults.errorMessages());
  }).before(async () => {
  ...
  ['template'].forEach((templateName) => { // 'menuItemTemplate'
    test(`Toolbar all widgets as ${templateName} appearance,locateInMenu=${locateInMenu}`, async (t) => {
      ...
      await t
        .expect(await takeScreenshot(`Toolbar all widgets as ${templateName} appearance${getThemePostfix()}.png`, targetContainer))
        .ok()
        .expect(compareResults.isValid())
        .ok(compareResults.errorMessages());
    }).before(async () => {
