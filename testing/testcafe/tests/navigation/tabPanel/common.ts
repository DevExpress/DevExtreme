...
test('Tab borders in TabPanel with long not stretched tabs', async (t) => {
  const { takeScreenshot, compareResults } = createScreenshotsComparer(t);

  await testScreenshot(t, takeScreenshot, 'TabPanel with long not stretched tabs.png', { element: '#container' });
