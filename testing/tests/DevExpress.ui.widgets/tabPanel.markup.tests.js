...
QUnit.module('TabPanel items', () => {
    ...
    [true, false].forEach(rtlEnabled => {
        QUnit.test(`rtlEnabled: ${rtlEnabled}, items: { title, icon } -> icon alignment`, function(assert) {
            ...
            assert.notEqual($title.find(`.${ICON_CLASS}`).get(0), undefined, 'element'); // for testing only
            assert.notEqual($title.find(`.${ICON_CLASS}`).get(0).getBoundingClientRect, undefined, 'getBoundingClientRect'); // for testing only
            assert.equal($title.find(`.${ICON_CLASS}`).get(0).tagName, 'I', 'getBoundingClientRect'); // for testing only
