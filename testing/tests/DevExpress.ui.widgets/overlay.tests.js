...
testModule("scrollable interaction", {
    ...
    if(devices.current().deviceType === "desktop") {
        class OverlayWithScrollableTestHelper {
            constructor(direction, targetElement) {
                this.direction = direction;
                this.wheelEventHandler = sinon.spy();
                this.isTargetContainer = targetElement === "container";
            }

            prepareOverlayWithScrollable() {
                this.$scrollable = $('<div><div id="unicID" style="height: 500px; width: 500px;"></div></div>');
                this.$overlay = $("#overlay").dxOverlay({
                    closeOnTargetScroll: true,
                    visible: true
                });
                $(this.$overlay.dxOverlay("$content")).append(this.$scrollable);
                this.scrollable = this.$scrollable.dxScrollable({
                    useNative: true,
                ...
                this.$container = this.$scrollable.find(".dx-scrollable-container");
            ...
            getNativePointerMock() {
                return nativePointerMock(this.isTargetContainer ? this.$container : this.$container.find("#unicID").eq(0));
            }

            getMaxScrollOffset(element) {
                var prop = this.direction === "vertical" || this.direction === "both" ? "Height" : "Width";
        ...
        ["vertical", "horizontal", "both"].forEach((direction) => {
            ["container", "content"].forEach((targetElement) => {
                test(`MouseWheelEvent prevented on bound: direction - ${direction}, scrollPosition: 0, mouseWheel - UP, target - ${targetElement}`, () => {
                    let helper = new OverlayWithScrollableTestHelper(direction, targetElement);
                    helper.prepareOverlayWithScrollable();
                    let pointerMock = helper.getNativePointerMock();
                ...
                test(`MouseWheelEvent prevented on bound: direction - ${direction}, scrollPosition: MAX, mouseWheel - DOWN, target - ${targetElement}`, () => {
                    let helper = new OverlayWithScrollableTestHelper(direction, targetElement);
                    helper.prepareOverlayWithScrollable();
                    const maxScrollOffset = helper.getMaxScrollOffset(helper.$container.get(0));
                    let pointerMock = helper.getNativePointerMock();

                    if(direction === "vertical") {
                        nativePointerMock(helper.$container).start().scroll(0, maxScrollOffset);
                        pointerMock.start().wheel(-30);
                        helper.checkAsserts({
                    ...
                    if(direction === "horizontal") {
                        nativePointerMock(helper.$container).start().scroll(maxScrollOffset, 0);
                        pointerMock.start().wheel(-30, true);
                        helper.checkAsserts({
                    ...
                    if(direction === "both") {
                        nativePointerMock(helper.$container).start().scroll(maxScrollOffset, maxScrollOffset);
                        pointerMock.start().wheel(-30);
                        helper.wheelEventHandler.reset();
                        pointerMock.start().wheel(-30, true);
                        helper.checkAsserts({
                ...
                test(`MouseWheelEvent not prevented before reaching the bound: direction - ${direction}, scrollPosition: center, mouseWheel - UP, target - ${targetElement}`, () => {
                    let helper = new OverlayWithScrollableTestHelper(direction, targetElement);
                    helper.prepareOverlayWithScrollable();
                    const maxScrollOffset = helper.getMaxScrollOffset(helper.$container.get(0));
                    let centerScrollPosition = Math.round(maxScrollOffset / 2);
                    let pointerMock = helper.getNativePointerMock();

                    if(direction === "vertical") {
                        nativePointerMock(helper.$container).start().scroll(0, centerScrollPosition);
                        pointerMock.start().wheel(30);
                        helper.checkAsserts({
                    ...
                    if(direction === "horizontal") {
                        nativePointerMock(helper.$container).start().scroll(centerScrollPosition, 0);
                        pointerMock.start().wheel(30, true);
                        helper.checkAsserts({
                    ...
                    if(direction === "both") {
                        nativePointerMock(helper.$container).start().scroll(centerScrollPosition, centerScrollPosition);
                        pointerMock.start().wheel(30);
                        helper.checkAsserts({
                ...
                test(`MouseWheelEvent not prevented before reaching the bound: direction - ${direction}, scrollPosition: center, mouseWheel - DOWN, target - ${targetElement}`, () => {
                    let helper = new OverlayWithScrollableTestHelper(direction, targetElement);
                    helper.prepareOverlayWithScrollable();
                    const maxScrollOffset = helper.getMaxScrollOffset(helper.$container.get(0));
                    let centerScrollPosition = Math.round(maxScrollOffset / 2);
