...
QUnit.module('Accessibility', ()=> {
    ...
    QUnit.test('list should have a selectByClick=true by default', function(assert) {
        const list = $('#list').dxList({ items: [1, 2], selectByClick: true }).dxList('instance');
        const isSelectByClick = list.option('selectByClick');

        assert.ok(isSelectByClick);
    });

    QUnit.test('list should have to change the value of selectByClick during initialization', function(assert) {
        const list = $('#list').dxList({ items: [1, 2], selectByClick: false }).dxList('instance');
        const isSelectByClick = list.option('selectByClick');

        assert.notOk(isSelectByClick);
    });

    QUnit.test('list should have to change the value of selectByClick after updating', function(assert) {
    ...
    QUnit.test('list item should not respond to a click', function(assert) {
        let actionCount = 0;
    ...
    QUnit.test('list item should respond to a click', function(assert) {
        let actionCount = 0;
        const $list = $('#list').dxList({
            items: [1, 2],
            showSelectionControls: true,
