...
QUnit.module('LoadIndicator markup', () => {
    QUnit.test('Basic markup initialization', function(assert) {
        const $indicator = $('#loadIndicator').dxLoadIndicator(), $indicatorWrapper = $indicator.find('.' + LOADINDICATOR_WRAPPER), $indicatorContent = $indicator.find('.' + LOADINDICATOR_CONTENT_CLASS);
    ...
    QUnit.test('LoadIndicator width custom dimensions', function(assert) {
        const $indicator = $('#loadIndicator').dxLoadIndicator({ width: 75, height: 75 }), indicatorElement = $indicator.get(0);
...
QUnit.module('Static load indicator', {
    ...
    QUnit.test('basic render', function(assert) {
        const $indicator = $('#loadIndicator').dxLoadIndicator({ visible: false, viaImage: false }), $indicatorWrapper = $indicator.find('.' + LOADINDICATOR_WRAPPER);
