...
QUnit.module("reordering decorator", {
    beforeEach: function() {
        this.clock = sinon.useFakeTimers();
        fx.off = true;
        this._originalSupportTransform = support.transform;
    },
    afterEach: function() {
        this.clock.restore();
        fx.off = false;
        support.transform = this._originalSupportTransform;
    }
});

QUnit.test("compatibility mode should be enabled for browsers without transform support", function(assert) {
    ...
    var $item = $list.find(toSelector(LIST_ITEM_CLASS)),
        pointer = reorderingPointerMock($item, this.clock, true);

    assert.ok(!$list.hasClass(LIST_REORDER_COMPATIBILITY_MODE_CLASS), "compatibility mode was not added on start");
    pointer.dragStart().drag(10);
    assert.ok($list.hasClass(LIST_REORDER_COMPATIBILITY_MODE_CLASS), "compatibility mode added");
    pointer.dragEnd();
    assert.ok(!$list.hasClass(LIST_REORDER_COMPATIBILITY_MODE_CLASS), "compatibility mode removed");
});

QUnit.test("compatibility mode should not be enabled for browsers without transform support", function(assert) {
    ...
    pointer.dragStart().drag(10);
    assert.ok(!$list.hasClass(LIST_REORDER_COMPATIBILITY_MODE_CLASS), "compatibility mode not added");
});
