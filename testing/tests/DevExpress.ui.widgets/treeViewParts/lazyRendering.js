...
[
    ...
    QUnit.test(`Nodes expanding should work with special charactes in id - ${testId}`, function(assert) {
        ...
        const parentItem = treeView.getElement().find('[aria-level="1"]');
        let childItem = treeView.getElement().find('[aria-level="2"]');
    ...
    QUnit.test(`Search should work with special charactes in the nodes ids - ${testId}`, function(assert) {
        ...
        let parentItem = treeView.getElement().find('[aria-label="item1"]');
        let childItem = treeView.getElement().find('[aria-label="item1_1"]');
        let anotherItem = treeView.getElement().find('[aria-label="item2"]');
