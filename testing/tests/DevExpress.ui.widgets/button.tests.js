...
QUnit.module('events subscriptions', () => {
    ...
    QUnit.test('onDisposing option', function(assert) {
        assert.expect(3);
        ...
        const button = $('#button').dxButton({
            ...
            onDisposing: (e) => {
                assert.ok(e.component, 'Component info should be passed');
                assert.ok(e.element, 'Element info should be passed');
                assert.strictEqual($(e.element).text(), 'test', 'Text is not removed');
            }
        }).dxButton('instance');
    ...
    QUnit.test('disposing event', function(assert) {
        assert.expect(3);
    ...
    QUnit.test('onOptionChanged option', function(assert) {
        ...
        button.option('text', 'test');
        button.option('text', 'new test');

        assert.strictEqual(eventHandler.callCount, 1, 'optionChanged handler was executed one time');
        const params = eventHandler.getCall(0).args[0];
        assert.strictEqual(params.value, 'new test', 'optionChanged handler has an correct value');
        assert.strictEqual(params.previousValue, 'test', 'optionChanged handler has an correct previous value');
    });

    QUnit.test('optionChanged event', function(assert) {
        const eventHandler = sinon.spy();
        ...
        button.on('optionChanged', eventHandler);
        button.option('text', 'test');
        button.option('text', 'new test');
    ...
    QUnit.test('onInitialized option', function(assert) {
        assert.expect(3);
        ...
        $('#button').dxButton({
            ...
            onInitialized: (e) => {
                assert.ok(e.component, 'Component info should be passed');
                assert.ok(e.element, 'Element info should be passed');
                assert.strictEqual($(e.element).text(), '', 'Text is not rendered');
            }
        });
    });
});
