...
import { registerPalette, getPalette, normalizePalette, Palette, DiscretePalette, _DEBUG_palettes, currentPalette, GradientPalette, getAccentColor } from "viz/palette";
...
QUnit.test('Disposing', function(assert) {
    // arrange
    var palette = this.Palette(['green', 'red'], { useHighlight: true });
...
QUnit.test('Palette is predefined', function(assert) {
    assert.strictEqual(this.Palette('default', { type: 'simpleSet' }).getNextColor(), this.palettes['default'].simpleSet[0], 'default');
    assert.strictEqual(this.Palette('Soft Pastel', { type: 'simpleSet' }).getNextColor(), this.palettes['soft pastel'].simpleSet[0], 'Soft Pastel');
});
...
QUnit.test("Throw warning when default palette name is used", function(assert) {
    ...
    try {
        this.Palette('default', { type: 'simpleSet' });
...
QUnit.test('Resolve theme palette', function(assert) {
    assert.strictEqual(this.Palette(undefined, { type: 'simpleSet' }, 'DARK VIOLET').getNextColor(), this.palettes['dark violet'].simpleSet[0], 'Soft Pastel by currentPalette');
    this.currentPalette('office');
    assert.strictEqual(this.Palette(undefined, { type: 'simpleSet' }, 'DARK VIOLET').getNextColor(), this.palettes['office'].simpleSet[0], 'Dark Violet');
});
...
QUnit.test('Custom palette by name', function(assert) {
    ...
    this.checkPalette(assert, this.Palette('Custom Palette', { extensionMode: "alternate" }), ['c1', 'c2', 'c3'], 'simpleSet');
    this.checkPalette(assert, this.Palette('Custom Palette', { type: 'indicatingSet', extensionMode: "alternate" }), ['d1', 'd2'], 'indicatingSet');
});
...
QUnit.test('Custom palette by unknown name', function(assert) {
    ...
    this.checkPalette(assert, this.Palette('Custom Palette 2', { extensionMode: "alternate" }), this.palettes['material'].simpleSet, 'simpleSet');
});

QUnit.test('Custom palette by array', function(assert) {
    this.checkPalette(assert, this.Palette(['a1', 'a2', 'a3'], { extensionMode: "alternate" }), ['a1', 'a2', 'a3']);
});

QUnit.test('Lightening palette', function(assert) {
    // act
    var palette = this.Palette(['green', 'red'], { useHighlight: true, extensionMode: "alternate" });
...
QUnit.test('Extrapolate without passing count', function(assert) {
    // act
    var palette = this.Palette(['green', 'red'], { extensionMode: "extrapolate" });
...
QUnit.test('Blend without passing count', function(assert) {
    // act
    var palette = this.Palette(['green', 'red', "yellow"], { extensionMode: "blend" });
...
QUnit.test('Blend with passing count. Keep last color in the end', function(assert) {
    // act
    var palette = this.Palette(['green', 'red', "yellow"], { extensionMode: "blend", keepLastColorInEnd: true });
...
QUnit.test('Lightening palette when color is too light', function(assert) {
    // act
    var palette = this.Palette(['white'], { useHighlight: true, extensionMode: "Alternate" });
...
QUnit.test('Darken palette when color is too dark', function(assert) {
    // act
    var palette = this.Palette(['black'], { useHighlight: true, extensionMode: "alternate" });
...
QUnit.test('palette is not valid', function(assert) {
    assert.strictEqual(this.DiscretePalette('', 2).getColor(0), this.palettes['material'].gradientSet[0], 'empty string');
    assert.strictEqual(this.DiscretePalette(undefined, 2).getColor(0), this.palettes['material'].gradientSet[0], 'undefined');
    assert.strictEqual(this.DiscretePalette(null, 2).getColor(0), this.palettes['material'].gradientSet[0], 'null');
    assert.strictEqual(this.DiscretePalette('test', 2).getColor(0), this.palettes['material'].gradientSet[0], 'unknown name');
});

QUnit.test('palette is predefined', function(assert) {
    assert.strictEqual(this.DiscretePalette('default', 2).getColor(0), this.palettes['default'].gradientSet[0], 'default');
    assert.strictEqual(this.DiscretePalette('Soft Pastel', 2).getColor(0), this.palettes['soft pastel'].gradientSet[0], 'Soft Pastel');
    assert.strictEqual(this.DiscretePalette('HARMONY LIGHT', 2).getColor(0), this.palettes['harmony light'].gradientSet[0], 'Harmony Light');
});

QUnit.test('resolve theme palette', function(assert) {
    assert.strictEqual(this.DiscretePalette(undefined, 2, 'DARK VIOLET').getColor(0), this.palettes['dark violet'].gradientSet[0], 'Soft Pastel by currentPalette');
    this.currentPalette('material');
    assert.strictEqual(this.DiscretePalette(undefined, 2, 'DARK VIOLET').getColor(0), this.palettes['material'].gradientSet[0], 'Dark Violet');
});

QUnit.test('palette is custom object', function(assert) {
    assert.strictEqual(this.DiscretePalette(['#0fad71', '#d82900'], 2).getColor(0), '#0fad71', 'custom palette 1');
    assert.strictEqual(this.DiscretePalette(['red', 'blue'], 2).getColor(0), '#ff0000', 'custom palette 2');
});

QUnit.test('palette size is 1', function(assert) {
    var start = new Color('#0f2e89'), end = new Color('#123fd7'),
        palette = this.DiscretePalette([start.toHex(), end.toHex()], 1);
...
QUnit.test('palette size is 2', function(assert) {
    var start = new Color('#ad8902'), end = new Color('#37e90a'),
        palette = this.DiscretePalette([start.toHex(), end.toHex()], 2);
...
QUnit.test('palette size is 3', function(assert) {
    var start = new Color('red'), end = new Color('blue'),
        palette = this.DiscretePalette([start.toHex(), end.toHex()], 3);
    assert.strictEqual(palette.getColor(0), start.toHex(), 'color 0');
...
QUnit.test('palette size is 51', function(assert) {
    var start = new Color('#000102'), end = new Color('#fff901'),
        palette = this.DiscretePalette([start.toHex(), end.toHex()], 51),
        i = 0;
    for(; i < 51; ++i) {
...
QUnit.test('palette size is not valid', function(assert) {
    assert.strictEqual(this.DiscretePalette('', 0).getColor(0), null, '0');
    assert.strictEqual(this.DiscretePalette('', 'test').getColor(0), null, 'test');
    assert.strictEqual(this.DiscretePalette('', -1).getColor(0), null, '-1');
});

QUnit.test('More than 2 colors in source', function(assert) {
    var colors = this.createColors(5),
        palette = this.DiscretePalette(colors, 5);
...
QUnit.test('More than 2 colors in source / greater then size / 1', function(assert) {
    var colors = this.createColors(7),
        palette = this.DiscretePalette(colors, 4);
...
QUnit.test('More than 2 colors in source / greater then size / 2', function(assert) {
    var colors = this.createColors(4),
        palette = this.DiscretePalette(colors, 3);
    assert.strictEqual(palette.getColor(0), colors[0], 'color 0');
...
QUnit.test('Create palette with current case', function(assert) {
    ...
    var p = this.Palette(undefined, { extensionMode: "alternate" });
