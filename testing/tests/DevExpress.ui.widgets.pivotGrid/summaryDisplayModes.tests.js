...
QUnit.test('Get data field', function(assert) {
    ...
    const cell = new this.Cell([this.grandTotalColumn], [this.grandTotalRow], this.data, this.descriptions, 1);
...
QUnit.test('Get cell value when there are several data fields get fieldValue by Fieldname', function(assert) {
    ...
    const cell = new this.Cell([this.grandTotalColumn], [this.grandTotalRow], this.data, this.descriptions, 0);
...
QUnit.test('get child rows', function(assert) {
    const childRows1 = new this.Cell([this.grandTotalColumn], [this.grandTotalRow], this.data, this.descriptions, 0).children('row');
    const childRows2 = new this.Cell([this.grandTotalColumn], [this.data.rows[1], this.grandTotalRow], this.data, this.descriptions, 0).children('row');
...
QUnit.test('get child columns', function(assert) {
    const childRows1 = new this.Cell([this.grandTotalColumn], [this.grandTotalRow], this.data, this.descriptions, 0).children('column');
    const childRows2 = new this.Cell([this.data.columns[0], this.grandTotalColumn], [this.data.rows[0], this.grandTotalRow], this.data, this.descriptions, 0).children('column');
    const childRows3 = new this.Cell([this.data.columns[0], this.grandTotalColumn], [this.data.rows[0], this.grandTotalRow], this.data, this.descriptions, 0).children();
...
QUnit.test('Slice. Columns', function(assert) {
    ...
    const cell = new this.Cell([this.data.columns[2].children[1], this.data.columns[2], this.grandTotalColumn], [this.grandTotalRow], this.data, this.descriptions, 0);
    const sliceCell = cell.slice('Field1', '1');
...
QUnit.test('Slice. Rows', function(assert) {
    const cell = new this.Cell([this.grandTotalColumn], [this.data.rows[1].children[0], this.data.rows[1], this.grandTotalRow], this.data, this.descriptions, 0);
    const sliceCell = cell.slice('Field3', 1991);
...
QUnit.test('Calculate display summary Type with preset', function(assert) {
    const summaryExpr = sinon.stub(summaryDictionary, 'percentOfColumnTotal');
    summaryExpr.returns('calculatedValue');
...
QUnit.test('Add percent format for percent display type', function(assert) {
    ...
    const setFieldProperty = sinon.spy(pivotGridUtils, 'setFieldProperty');
...
QUnit.test('Second calculation leads to same results', function(assert) {
    ...
    applyDisplaySummaryMode(this.descriptions, this.data);
    const value = this.data.values[0][6][0];
...
QUnit.test('RunningTotal by row', function(assert) {
    ...
    // assert
    const values = this.data.values;
    const firstColumn = $.map(values, function(row) { return row[0]; });
...
QUnit.module('summary display mode presets', {
    beforeEach: function() {
        const Cell = summaryDisplayModes.Cell;

        this.stubCell = function() {
            const stub = sinon.createStubInstance(Cell);
...
QUnit.test('Absolute variation. Crossgrouping', function(assert) {
    const cell = this.stubCell();
    const prevCell = this.stubCell();
    const calculateSummaryCallback = this.getExpression('absoluteVariation', true);

    prevCell.value.returns(2);

    cell.prev.withArgs('column', true).returns(prevCell);
    const resultWithPrevCell = calculateSummaryCallback(cell);

    cell.prev.withArgs('column', true).returns(null);
    const resultWithoutPrevCell = calculateSummaryCallback(cell);

    cell.prev.withArgs('column', true).returns(prevCell);
    prevCell.value.returns(null);
    cell.value.returns(null);
    const resultWithNullValues = calculateSummaryCallback(cell);

    prevCell.value.returns(null);
    cell.value.returns(10);
    const resultWithPrevCellNullValue = calculateSummaryCallback(cell);

    prevCell.value.returns(2);
    cell.value.returns(null);
    const resultWithCellNullValue = calculateSummaryCallback(cell);

    prevCell.value.returns(2);
    cell.value.returns(2);
    const resultWithEqualValues = calculateSummaryCallback(cell);
...
QUnit.test('Absolute variation', function(assert) {
    const cell = this.stubCell();
    const prevCell = this.stubCell();
    const calculateSummaryCallback = this.getExpression('absoluteVariation', false);
...
QUnit.test('Percent variation. CrossGrouping', function(assert) {
    const cell = this.stubCell();
    const prevCell = this.stubCell();
    const calculateSummaryCallback = this.getExpression('percentVariation', true);
    ...
    const resultWithZeroPrevValue = calculateSummaryCallback(cell);
...
QUnit.test('Percent variation.', function(assert) {
    const cell = this.stubCell();
    const prevCell = this.stubCell();
    const calculateSummaryCallback = this.getExpression('percentVariation', false);
...
QUnit.test('Percent of row grand total', function(assert) {
    const cell = this.stubCell();
    const totalCell = this.stubCell();
    const calculateSummaryCallback = this.getExpression('percentOfRowGrandTotal');

    totalCell.value.returns(20);

    cell.grandTotal.withArgs('column').returns(totalCell);
    const resultWithTotalCell = calculateSummaryCallback(cell);
...
QUnit.test('Percent of column grand total', function(assert) {
    const cell = this.stubCell();
    const totalCell = this.stubCell();
    const calculateSummaryCallback = this.getExpression('percentOfColumnGrandTotal');
...
QUnit.test('Percent of grandTotal', function(assert) {
    const cell = this.stubCell();
    const totalCell = this.stubCell();
    const calculateSummaryCallback = this.getExpression('percentOfGrandTotal');
...
QUnit.test('Percent of row', function(assert) {
    const cell = this.stubCell();
    const parentCell = this.stubCell();
    const calculateSummaryCallback = this.getExpression('percentOfRowTotal');

    parentCell.value.returns(20);

    cell.parent.withArgs('column').returns(parentCell);
    const resultWithParentCell = calculateSummaryCallback(cell);
    ...
    const resultWhenNoParent = calculateSummaryCallback(cell);// current cell is grandTotal by row
...
QUnit.test('Percent of Total row should be null when it\'s value is 0', function(assert) {
    ...
    const result = calculateSummaryCallback(cell);
    // assert
...
QUnit.test('Percent of column', function(assert) {
    const cell = this.stubCell();
    const parentCell = this.stubCell();
    const calculateSummaryCallback = this.getExpression('percentOfColumnTotal');
    ...
    const resultWhenNoParent = calculateSummaryCallback(cell);// current cell is grandTotal by column
...
QUnit.test('empty column', function(assert) {
    ...
    pivotGridUtils.foreachTree(this.data.columns, function(items) {
        const path = pivotGridUtils.createPath(items).join('.');
        if(path === '1') {
...
QUnit.test('empty row', function(assert) {
    ...
    pivotGridUtils.foreachTree(this.data.rows, function(items) {
        const path = pivotGridUtils.createPath(items);
...
QUnit.test('createMockSummaryCell with custom calculateSummaryValue', function(assert) {
    ...
    const summaryCell = summaryDisplayModes.createMockSummaryCell(descriptions, fields, {});
...
const dataWithTwoValues = $.extend(true, {}, data);
