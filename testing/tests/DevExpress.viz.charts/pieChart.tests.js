...
var environment = {
        beforeEach: function() {
            ...
            that.layoutManager.applyPieChartSeriesLayout.returns({ radiusInner: 0, radiusOuter: 300, centerX: 100, centerY: 200, canvas: {} });
...
(function mainTest() {
    ...
    function checkCorrectPosition(assert, correctPos, x, y, outer, inner) {
        assert.equal(correctPos.centerX, x, "centerX");
        assert.equal(correctPos.centerY, y, "centerY");
        assert.equal(correctPos.radiusOuter, outer, "radiusOuter");
        assert.equal(correctPos.radiusInner, inner, "radiusInner");
        assert.deepEqual(correctPos.canvas, {}, "canvas");
    }
    ...
    QUnit.test("draw series", function(assert) {
        ...
        $.each(series, function(i, singleSeries) {
            ...
            checkCorrectPosition(assert, singleSeries.correctPosition.getCall(0).args[0], 100, 200, 300, 0);
    ...
    QUnit.test("one of the series in not visible", function(assert) {
        ...
        checkCorrectPosition(assert, series[1].correctPosition.getCall(0).args[0], 100, 200, 300, 0);
    ...
    QUnit.test("Legend callback", function(assert) {
        ...
        var legendActionCallback = commons.getLegendStub().stub("getActionCallback").returns(action),
            legendCallback = seriesMockData.series[0].legendCallback;
        //act
        legendCallback();
