...
QUnit.test("dxpointermove on series, click", function(assert) {
    ...
    assert.deepEqual(this.options.eventTrigger.withArgs("pointClick").lastCall.args[1], { target: this.point, jQueryEvent: clickEvent });
...
QUnit.test("legendClick", function(assert) {
    ...
    assert.deepEqual(this.options.eventTrigger.withArgs("legendClick").lastCall.args[1], { target: this.series, jQueryEvent: event });
    ...
    assert.ok(this.options.eventTrigger.withArgs("seriesClick").calledOnce);
    assert.deepEqual(this.options.eventTrigger.withArgs("seriesClick").lastCall.args[1], { target: this.series, jQueryEvent: event }, "series event arg");
});
...
QUnit.test("click on argument axis element. Axis hoverMode is 'none'", function(assert) {
    ...
    assert.deepEqual(this.options.eventTrigger.withArgs("argumentAxisClick").lastCall.args[1], { argument: "argument1", jQueryEvent: event });
});
...
QUnit.test("point click", function(assert) {
    ...
    assert.ok(this.options.eventTrigger.withArgs("pointClick").calledOnce);
    strictEqualForAllFields(assert, this.options.eventTrigger.withArgs("pointClick").lastCall.args[1], { target: this.point, jQueryEvent: event });
});
...
QUnit.test("legend item click. One series", function(assert) {
    ...
    assert.strictEqual(legendClick.lastCall.args[1].jQueryEvent, event, "event");
});
