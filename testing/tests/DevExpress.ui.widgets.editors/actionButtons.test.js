...
module('button collection', () => {
    ...
    module('button', () => {
        ...
        module('fields', () => {
            ...
            test('custom button should have parentIndependentState option as true', function(assert) {
                const $textBox = $('<div>').dxTextBox({ buttons: [{ name: 'name1' }] });
                const buttons = getTextEditorButtons($textBox);
                const button = buttons.$after.eq(0).dxButton('instance');

                assert.strictEqual(button.option('parentIndependentState'), true, 'button has parentIndependentState option');
            });
        });
    });
});
