...
QUnit.test("dxDropDownEditor must have a button", function(assert) {
    assert.ok(this.dropDownEditor._$dropButton);
});

QUnit.test("button must be decorated with DROP_DOWN_EDITOR_BUTTON_CLASS", function(assert) {
    assert.strictEqual(this.rootElement.find("." + DROP_DOWN_EDITOR_BUTTON_CLASS)[0], this.dropDownEditor._$dropButton[0]);
    assert.ok(this.dropDownEditor._$dropButton.hasClass(DROP_DOWN_EDITOR_BUTTON_CLASS));
});
...
QUnit.test("clicking the input must not close the dropdown", function(assert) {
    this.dropDownEditor._input().get(0).focus = function() { }; // NOTE: hack for IE8 to prevent triggering window.scroll on input.focus
    this.dropDownEditor.open();
...
QUnit.test("clicking the button must correctly close the dropdown", function(assert) {
    this.dropDownEditor._input().get(0).focus = function() { }; // NOTE: hack for IE8 to prevent triggering window.scroll on input.focus
    this.dropDownEditor.open();
    pointerMock(this.dropDownEditor._$dropButton).click();
    assert.ok(!this.dropDownEditor.option("opened"));
});
...
QUnit.test("clicking the button descendants must also correctly close the dropdown", function(assert) {
    this.dropDownEditor._input().get(0).focus = function() { }; // NOTE: hack for IE8 to prevent triggering window.scroll on input.focus
    this.dropDownEditor.open();
    pointerMock(this.dropDownEditor._$dropButton.find("." + DROP_DOWN_EDITOR_BUTTON_ICON)).click();
    assert.ok(!this.dropDownEditor.option("opened"));
});
...
QUnit.test("if button is visible, the root element must be marked with the DROP_DOWN_EDITOR_BUTTON_VISIBLE class", function(assert) {
    assert.ok(this.rootElement.hasClass(DROP_DOWN_EDITOR_BUTTON_VISIBLE));
});
...
QUnit.testInActiveWindow("input is focused by click on dropbutton", function(assert) {
    var $dropDownEditor = $("#dropDownEditorLazy").dxDropDownEditor({
        focusStateEnabled: true
    });
    var $dropButton = $dropDownEditor.find(".dx-dropdowneditor-button");
    $dropButton.trigger("dxclick");
...
QUnit.test("contentTemplate as render", function(assert) {
    $("#dropDownEditorLazy").dxDropDownEditor({
        contentTemplate: function(value) {
            return $("<div>").text(this.option("value"));
        },
        value: "test",
        opened: true
    });
...
QUnit.test("DropButton state after drop readOnly editor's state", function(assert) {
    var $dropDownEditor = $("#dropDownEditorLazy").dxDropDownEditor({
        readOnly: true
    });
    ...
    var dropButton = $dropDownEditor.find(".dx-dropdowneditor-button").dxButton("instance");

    assert.equal(dropButton.option("disabled"), false, "dropButton is not disabled");
});
