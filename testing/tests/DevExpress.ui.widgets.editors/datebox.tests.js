...
QUnit.test("T204179 - dxDateBox should not render dropButton only for generic device when pickerType is 'native'", function(assert) {
    var $dateBox = $("#dateBox").dxDateBox({
            pickerType: "native"
        }),
        $dropButton = $dateBox.find(".dx-dropdowneditor-button"),
        expectedButtonsNumber = devices.real().platform === "generic" ? 0 : 1;

    assert.equal($dropButton.length, expectedButtonsNumber, "correct readOnly value");
});
...
QUnit.test("Clicking _calendarContainer must not close dropDown", function(assert) {
    this.fixture.input.get(0).focus = function() { }; // NOTE: hack for IE8 to prevent triggering window.scroll on input.focus
    this.fixture.dateBox.open();
...
QUnit.test("DateBox should not take current date value at the opening if value is null", function(assert) {
    ...
    var instance = $dateBox.dxDateBox("instance");
    var $dropButton = $dateBox.find(".dx-dropdowneditor-button");
