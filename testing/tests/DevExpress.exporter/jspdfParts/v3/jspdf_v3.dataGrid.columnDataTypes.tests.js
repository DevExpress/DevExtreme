...
const JSPdfColumnDataTypesTests = {
    runTests(moduleConfig, createMockPdfDoc, createDataGrid) {
        QUnit.module('Column data types', moduleConfig, () => {
            ...
            QUnit.test('Columns.dataType: string, values: [\'1\', \'2\'], selectedRowKeys: [ds[1]]', function(assert) {
                ...
                exportDataGrid(doc, dataGrid, { topLeft: { x: 10, y: 15 }, columnWidths: [ 100 ], selectedRowsOnly: true }).then(() => {
                    // doc.save(assert.test.testName + '.pdf');
                    assert.deepEqual(doc.__log, expectedLog);
                    done();
                });
            ...
            QUnit.test('Columns.dataType: string, unbound', function(assert) {
                ...
                exportDataGrid(doc, dataGrid, { topLeft: { x: 10, y: 15 }, columnWidths: [ 100, 100, 100, 100, 100 ] }).then(() => {
                    // doc.save(assert.test.testName + '.pdf');
                    assert.deepEqual(doc.__log, expectedLog);
                    done();
                });
            ...
            QUnit.test('Columns.dataType: number, unbound', function(assert) {
                ...
                exportDataGrid(doc, dataGrid, { topLeft: { x: 10, y: 15 }, columnWidths: [ 70, 70, 70, 70, 70, 70, 70 ] }).then(() => {
                    // doc.save(assert.test.testName + '.pdf');
                    assert.deepEqual(doc.__log, expectedLog);
                    done();
                });
            ...
            QUnit.test('Columns.dataType: date, values: [ 10/9/2019, 10/9/2020], selectedRowKeys: [ds[1]]', function(assert) {
                ...
                exportDataGrid(doc, dataGrid, { topLeft: { x: 10, y: 15 }, columnWidths: [ 200 ], selectedRowsOnly: true }).then(() => {
                    // doc.save(assert.test.testName + '.pdf');
                    assert.deepEqual(doc.__log, expectedLog);
                    done();
                });
...
export { JSPdfColumnDataTypesTests };
