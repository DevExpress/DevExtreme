...
QUnit.test("unable to redefine some system methods in IE8", function(assert) {
    $.each([
        "toString",
        "toLocaleString",
        "valueOf"
        ...
        var methodCalled = 0,
            members = {};
        members[this] = function() { methodCalled++; };
        var subClass = Class.inherit(members);
        new subClass()[this]();
        assert.equal(methodCalled, 1, "'" + this + "' method");
    });
});
