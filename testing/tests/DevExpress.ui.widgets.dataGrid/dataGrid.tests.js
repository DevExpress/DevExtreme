...
QUnit.module("optionChanged", {
    ...
    QUnit.test("Event should be fired when changing the column.visible option", function(assert) {
        ...
        //assert
        assert.strictEqual(spy.callCount, 1, "call count the onOptionChanged");
        assert.deepEqual(spy.getCall(0).args[0], { component: dataGrid, element: dataGrid.element(), name: "columns", fullName: "columns[0].visible", previousValue: true, value: false }, "onOptionChanged args");
    });
    ...
    QUnit.test("Event should be fired when grouping", function(assert) {
        ...
        //act
        dataGrid.getController("columns").moveColumn(0, -1, "headers", "group");

        //assert
        groupIndexCall = spy.getCalls().filter(function(params) {
            if(params.args[0].fullName === "columns[0].groupIndex") {
                return true;
            }
        })[0];
        assert.deepEqual(groupIndexCall.args[0], { component: dataGrid, element: dataGrid.element(), name: "columns", fullName: "columns[0].groupIndex", value: 0 }, "onOptionChanged args");
    });
    ...
    QUnit.test("Event should be fired when sorting", function(assert) {
        ...
        dataGrid.option("columns", [{ dataField: "field1", sortOrder: "asc" }, "field2"]);
        spy.reset();

        //act
        dataGrid.getController("columns").changeSortOrder(0, "desc");

        //assert
        assert.deepEqual(spy.getCall(0).args[0], { component: dataGrid, element: dataGrid.element(), name: "columns", fullName: "columns[0].sortOrder", previousValue: "asc", value: "desc" }, "onOptionChanged args");
    });
    ...
    QUnit.test("Checking arguments when sorting a banded column", function(assert) {
        ...
        dataGrid.option("columns", ["field1", {
        ...
        //act
        dataGrid.getController("columns").columnOption("field3", "sortOrder", "desc");

        //assert
        assert.deepEqual(spy.getCall(0).args[0], { component: dataGrid, element: dataGrid.element(), name: "columns", fullName: "columns[1].columns[1].columns[0].sortOrder", value: "desc" }, "onOptionChanged args");
    });
    ...
    QUnit.test("Event should be fired when changing the pageSize", function(assert) {
        ...
        dataGrid.pageSize(4).done(function() {
            //assert
            assert.deepEqual(spy.getCall(0).args[0], { component: dataGrid, element: dataGrid.element(), name: "paging", fullName: "paging.pageSize", value: 4, previousValue: 2 }, "onOptionChanged args");
        });
    });
    ...
    QUnit.test("Event should be fired when changing the pageIndex", function(assert) {
        ...
        dataGrid.pageIndex(1).done(function() {
            //assert
            assert.deepEqual(spy.getCall(0).args[0], { component: dataGrid, element: dataGrid.element(), name: "paging", fullName: "paging.pageIndex", value: 1 }, "onOptionChanged args");
        });
    });
});
