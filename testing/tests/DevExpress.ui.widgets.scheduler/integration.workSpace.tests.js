...
QUnit.test('Group header should contain group header content with right height, groupOrientation = vertical', function(assert) {
    ...
    const $headerContents = header.find('.dx-scheduler-group-header-content');
    const cellHeight = this.instance.$element().find('.dx-scheduler-date-table-cell').eq(1).outerHeight();
