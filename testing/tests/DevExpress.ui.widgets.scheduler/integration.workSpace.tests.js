    ...
    QUnit.module('Cells selection', {
        ...
        ['virtual', 'standard'].forEach((scrollingMode) => {
            ...
            [{
                ...
                focusedCellsCount, cellFromAnotherGroup, view,
            }) => {
                QUnit.test(`Mouse Multiselection should work correctly with ${view} when it is grouped vertically and scrolling is ${scrollingMode}`, function(assert) {
                    const scheduler = createWrapper({
                        views: [{
                            type: view,
                            intervalCount,
                    ...
                    const { workSpace } = scheduler;
                    workSpace.selectCells(startCell, endCell, false);

                    assert.equal(workSpace.getSelectedCells().length, focusedCellsCount, 'the amount of focused cells is correct');
                    assert.ok(workSpace.getCell(startCell).hasClass(SELECTED_CELL_CLASS), 'the start cell is focused');
                    assert.ok(workSpace.getCell(endCell).hasClass(SELECTED_CELL_CLASS), 'the end cell is focused');
                    intermediateCells.forEach((cell) => {
                        assert.ok(workSpace.getCell(cell).hasClass(SELECTED_CELL_CLASS), 'intermediate cell is focused');
                    });

                    const cell = workSpace.getCell(cellFromAnotherGroup).get(0);
                    $(workSpace.getWorkSpace()).trigger($.Event('dxpointermove', { target: cell, which: 1 }));

                    assert.equal(workSpace.getSelectedCells().length, focusedCellsCount, 'the amount of focused cells has not changed');
                    assert.ok(workSpace.getCell(startCell).hasClass(SELECTED_CELL_CLASS), 'the start cell is still focused');
                    assert.ok(workSpace.getCell(endCell).hasClass(SELECTED_CELL_CLASS), 'the end cell is still focused');
                    intermediateCells.forEach((cell) => {
                        assert.ok(workSpace.getCell(cell).hasClass(SELECTED_CELL_CLASS), 'intermediate cell is still focused');
                    });
                    assert.notOk(workSpace.getCell(cellFromAnotherGroup).hasClass(SELECTED_CELL_CLASS), 'cell from another group is not focused');

                    $(workSpace.getWorkSpace()).trigger($.Event('dxpointerup', { target: cell, which: 1 }));
                });
            });
        });
    });
