...
QUnit.test("Many dropDown appts with one multi day task should be grouped correctly", function(assert) {
    ...
    this.scheduler.appointments.compact.click();
    assert.equal(this.scheduler.tooltip.getItemCount(), 7, "There are 7 drop down appts");
});
...
QUnit.test("DropDown appointments should not be duplicated when items option change (T503748)", function(assert) {
    ...
    this.scheduler.appointments.compact.click();
    assert.equal(this.scheduler.tooltip.getItemCount(), 3, "There are 3 drop down appts");
});
...
QUnit.test("Long appointment part should have right width on timeline month view (T678380)", function(assert) {
    var appointment = {
