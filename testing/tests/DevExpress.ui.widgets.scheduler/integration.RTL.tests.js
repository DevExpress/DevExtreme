        ...
        test('Appointment should have correct position with multiple resources if rtlEnabled is true (T803275)', function(assert) {
            ...
            views.forEach(view => {
                ...
                [getAppointment(0), getAppointment(1)].forEach((appointment, index) => {
                    ...
                    assert.roughEqual(Math.round(position.left), expectedValue[index].left, 3, `left position of ${index} appointment should be correct in ${view} view`);
                    assert.roughEqual(Math.round(position.top), expectedValue[index].top, 3, `top position of ${index} appointment should be correct in ${view} view`);
                });
            });
        });
    }
