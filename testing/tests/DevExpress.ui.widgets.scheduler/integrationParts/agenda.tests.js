...
QUnit.test("Scheduler should have a right rendering strategy for agenda view", function(assert) {
    ...
    assert.equal(appointments.option("renderingStrategy"), "agenda", "Strategy is OK");
});
...
QUnit.test("Agenda should contain a right appointment quantity", function(assert) {
    ...
    this.instance.element().find(".dx-scheduler-appointment").each(function() {
        ...
        assert.notOk(apptData.appointmentData);
...
QUnit.test("Particular recurrence appt data calculation", function(assert) {
    ...
    var renderingStrategy = this.instance.getAppointmentsInstance()._renderingStrategy;
    var rows = [
        [0, 1, 0, 2, 1, 1, 1],
        [3, 0, 1, 0, 1, 1, 1]
    ];
...
QUnit.test("Appointment data of long-appointments should be the same", function(assert) {
    this.createInstance({
        views: ["agenda"],
        currentView: "agenda",
        currentDate: new Date(2016, 1, 24),
    ...
    assert.strictEqual($appointments.eq(0).data("dxItemData"), $appointments.eq(1).data("dxItemData"), "Appointments data is OK");
});
