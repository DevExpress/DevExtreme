...
QUnit.test("DataSource option should be passed to the appointments collection", function(assert) {
    ...
    assert.deepEqual(this.instance.getAppointmentsInstance().option("items"), this.instance.option("dataSource").items(), "Items option is correct");
});
...
QUnit.test("Tasks should be filtered by resources if groups are changed", function(assert) {
    var tasks = [
            ...
            { text: "b", startDate: new Date(2015, 2, 16), endDate: new Date(2015, 2, 16, 0, 30), ownerId: 3, roomId: [1, 2] }, // false
            { text: "c", startDate: new Date(2015, 2, 16), endDate: new Date(2015, 2, 16, 0, 30), ownerId: 1, roomId: [1, 2, 3] } // false
    ];
    var dataSource = new DataSource({
        store: tasks
    });
    ...
    assert.equal($appointments.length, 2, "There are four appointment");
    assert.deepEqual($appointments.eq(0).data("dxItemData"), tasks[1], "The first appointment data is OK");
    assert.deepEqual($appointments.eq(1).data("dxItemData"), tasks[1], "The second appointment dat is OK");
});
...
QUnit.test("Drag task that contains timestamps", function(assert) {
    var data = new DataSource({
        store: [
            {
                text: "Task 1",
...
QUnit.test("DropDown appointment button should have correct coordinates", function(assert) {
    ...
    sinon.stub(this.instance.getAppointmentsInstance()._renderingStrategy, "_getMaxNeighborAppointmentCount").returns(4);
