...
test('The vertical scroll bar of the container\'s parent should not be displayed when the grid has no height, virtual scrolling and state storing are enabled', async (t) => {
  ...
  await testScreenshot(t, takeScreenshot, 'T1129106-treelist-virtual-scrolling-1');

  // act
  await scrollWindowTo({ top: 10000000 });

  await t.expect(treeList.isReady()).ok();
  await testScreenshot(t, takeScreenshot, 'T1129106-treelist-virtual-scrolling-2');

  // act
  await scrollWindowTo({ top: 0 });

  await t.expect(treeList.isReady()).ok();
  await testScreenshot(t, takeScreenshot, 'T1129106-treelist-virtual-scrolling-3');
...
safeSizeTest('All items should be selected after select all and scroll down', async (t) => {
  ...
  await testScreenshot(t, takeScreenshot, 'T1189118-treelist-select-all-with-virtual-scrolling-1', { element: treeList.element });

  // act
  await treeList.scrollTo(t, { y: 300 });

  await testScreenshot(t, takeScreenshot, 'T1189118-treelist-select-all-with-virtual-scrolling-2', { element: treeList.element });
