...
test('The shading should alternate correctly after expanding the node when repaintChangesOnly is enabled', async (t) => {
  ...
  await testScreenshot(t, takeScreenshot, 'T1291705-row-alternation-after-expanding-node-when-repaintChangesOnly=true', { element: treeList.element });
...
test('The shading should alternate correctly after expanding the node when repaintChangesOnly and old fixed columns are enabled', async (t) => {
  ...
  await testScreenshot(t, takeScreenshot, 'T1291705-row-alternation-after-expanding-node-when-there-is-fixed-column-and-repaintChangesOnly=true', { element: treeList.element });
