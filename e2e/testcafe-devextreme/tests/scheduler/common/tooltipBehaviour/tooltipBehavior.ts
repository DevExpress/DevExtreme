...
test.meta({ browserSize: DEFAULT_BROWSER_SIZE })('Collector tooltip should stay visible when one appointment is deleted and items remain', async (t) => {
  const scheduler = new Scheduler('#container');
  const collector = scheduler.collectors.find('2 more');
  const { appointmentTooltip } = scheduler;
...
test.meta({ browserSize: DEFAULT_BROWSER_SIZE })('Collector tooltip should hide when last item is deleted', async (t) => {
  const scheduler = new Scheduler('#container');
