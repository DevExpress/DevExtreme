  ...
  [20, 30].forEach((iconSize) => {
    [false, true].forEach((limitedWidth) => {
      test(`Checkbox styles, rtlEnabled=${rtlEnabled} iconSize=${iconSize} limitedWidth=${limitedWidth}`, async (t) => {
        ...
        await testScreenshot(t, takeScreenshot, `CheckBox states rtlEnabled=${rtlEnabled} iconSize=${iconSize} limitedWidth=${limitedWidth}.png`, { element: '#container', shouldTestInCompact: true, theme: getFullThemeName().replace('light', 'dark') });
