...
borderConfigs.forEach(({ showRowLines, showColumnLines, showBorders }) => {
  rtlAndRowAltConfigs.forEach(({ rtlEnabled, rowAlternationEnabled }) => {
    safeSizeTest(`Sticky columns with left position (showRowLines = ${showRowLines}, showColumnLines = ${showColumnLines}, showBorders = ${showBorders}, rtlEnabled = ${rtlEnabled}, rowAlternationEnabled = ${rowAlternationEnabled})`, async (t) => {
      ...
      await takeScreenshot(`sticky-columns-with-left-position-1(showRowLines_=_${showRowLines}_showColumnLines_=_${showColumnLines}_showBorders_=_${showBorders}_rtlEnabled_=_${rtlEnabled}_rowAlternationEnabled_=_${rowAlternationEnabled}).png`, dataGrid.element);

      // act
      await dataGrid.scrollTo(t, { x: rtlEnabled ? 0 : 10000 });

      await takeScreenshot(`sticky-columns-with-left-position-2(showRowLines_=_${showRowLines}_showColumnLines_=_${showColumnLines}_showBorders_=_${showBorders}_rtlEnabled_=_${rtlEnabled}_rowAlternationEnabled_=_${rowAlternationEnabled}).png`, dataGrid.element);
