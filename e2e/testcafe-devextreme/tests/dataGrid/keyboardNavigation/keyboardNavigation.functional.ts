...
// T1216832
test('Navigation should not get stuck in the grid when the first cell is fixed and a template with links is specified for it', async (t) => {
  const dataGrid = new DataGrid('#container');
  const headers = dataGrid.getHeaders();
  const $myInput = Selector('#myinput');
  ...
  async function checkNavigationOfAllCells(): Promise<void> {
    ...
    let $fixedCell = dataGrid.getFixedDataRow(0).getDataCell(0);
    let $link = $fixedCell.element.find('a');

    await t
      .expect($fixedCell.element.focused)
      .ok()
      .pressKey('tab')
      .expect($link.focused)
      .ok()
    ...
    $fixedCell = dataGrid.getFixedDataRow(1).getDataCell(0);
    $link = $fixedCell.element.find('a');
  ...
  await t
    .pressKey('tab')
    .expect($myInput.focused)
    .ok();
}).before(async () => {
  await ClientFunction(() => {
    $('<input id="myinput">').appendTo('body');
  })();
