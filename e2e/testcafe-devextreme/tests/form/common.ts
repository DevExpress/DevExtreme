...
['auto', 'bottom', 'left', 'right', 'top'].forEach((validationMessagePosition) => {
  test.meta({ unstable: true })(`The validation message overlay for DateRangeBox with position ${validationMessagePosition} should be correctly positioned both before and after opening`, async (t) => {
    const { takeScreenshot, compareResults } = createScreenshotsComparer(t);
