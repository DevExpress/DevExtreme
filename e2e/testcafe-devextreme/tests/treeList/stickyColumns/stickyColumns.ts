...
safeSizeTest('Header hover should display correctly when there are fixed columns', async (t) => {
  ...
  await takeScreenshot('treelist_header_hover_with_fixed_columns_(generic.light).png', treeList.element);
...
test.meta({ unstable: true })('Row hover should display correctly when there are fixed columns', async (t) => {
  ...
  await takeScreenshot('treelist_row_hover_with_fixed_columns_(generic.light).png', treeList.element);
