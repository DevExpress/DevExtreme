...
test('TreeList - Expand/collapse buttons are too close to column borders if the first column is a boolean column (T1223168)', async (t) => {
  ...
  await t
    .expect(await takeScreenshot('T1223168-expandable-generic.light', treeList.element))
    .ok()
...
test.meta({ unstable: true })('TreeList screenshot when the first cell has a template', async (t) => {
  ...
  await t
    ...
    .ok()
    .expect(await takeScreenshot('T1221037-cell-with-template-generic.light', treeList.element))
    .ok()
    .expect(compareResults.isValid())
    .ok(compareResults.errorMessages());
    ...
    [false, true].forEach((rtlEnabled) => {
      test(
        ...
        async (t) => {
          ...
          await t
            .expect(await takeScreenshot(`markup-selection=${selectionMode}-rtl=${rtlEnabled}-content=${contentType} (generic.light)`, treeList.element))
            .ok()
            .expect(compareResults.isValid())
            .ok(compareResults.errorMessages());
        },
