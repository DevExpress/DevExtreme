...
[
  ...
  test.only(`Rows content dont hide under vertical scrollbar when scrolling{useNative=${useNative}},height=100% (450px) (T1290313)`, async (t) => {
    const { takeScreenshot, compareResults } = createScreenshotsComparer(t);

    const { getInstance } = new PivotGrid('#container');
    ...
    await testScreenshot(
      t,
      takeScreenshot,
      `PivotGrid rows content height=100%(450px),useNative=${useNative}.png`,
      { element: '#container' },
    );
    await t
      .expect(compareResults.isValid())
    ...
    await insertStylesheetRulesToPage('#parentContainer { height: 450px; }');
