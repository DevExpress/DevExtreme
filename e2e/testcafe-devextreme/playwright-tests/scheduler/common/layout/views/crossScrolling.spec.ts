...
test('Scrollable synchronization should work after changing current date (T1027231)', async ({ page }) => {
  ...
  await page.evaluate((d) => $('#container').dxScheduler('instance').scrollTo(new Date(d)), (new Date(2021, 4, 15).toISOString()), { priorityId: 2 });
...
test('Scrollable should be prepared correctly after change visibility (T1032171)', async ({ page }) => {
  ...
  await page.evaluate((d) => $('#container').dxScheduler('instance').scrollTo(new Date(d)), (new Date(2021, 1, 12).toISOString()));
