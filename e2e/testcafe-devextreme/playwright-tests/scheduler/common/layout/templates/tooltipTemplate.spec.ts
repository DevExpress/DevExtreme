  ...
  test('appointmentTooltipTemplate layout should be rendered right', async ({ page }) => {
    await page.evaluate(() => {
      ...
      ($('#container') as any).dxScheduler({
        dataSource: [{ startDate: new Date(2017, 4, 25, 0, 30), endDate: new Date(2017, 4, 25, 2, 30) }],
        views: ['workWeek'], currentView: 'workWeek', currentDate: new Date(2017, 4, 25),
        appointmentTooltipTemplate(appointment: any) {
    ...
    await page.locator('.dx-scheduler-appointment').first().click();
    await testScreenshot(page, 'appointment-tooltip-template.png', { element: page.locator('.dx-scheduler') });
  });
});
