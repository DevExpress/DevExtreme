...
test('Agenda test appointment state', async ({ page }) => {
  // Scheduler on '#container'
    await (page.locator('.dx-scheduler-appointment').filter({ hasText: 'Final Budget Review' }).hover().element);
  await testScreenshot(page, 'agenda-layout-appointment-state-hover.png');

  await (page.locator('.dx-scheduler-appointment').filter({ hasText: 'New Brochures' }).click().element);
