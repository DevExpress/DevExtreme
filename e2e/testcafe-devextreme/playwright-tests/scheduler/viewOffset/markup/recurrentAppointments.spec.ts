...
test.describe('Offset: Markup recurrent appointments', () => {
  ...
  [
    ...
    [0, 735, -735].forEach((offset) => {
      [
        ...
        test(`Recurrence appointments render (view: ${views[0].type}, offset: ${offset}, start: ${startDayHour}, end: ${endDayHour}, firstDay: ${(views[0] as any).firstDayOfWeek})`, async ({ page }) => {
  ...
  [
    ...
    ].forEach(({ dataSource, offset, startDayHour, endDayHour }) => {
      test(`Recurrence appointments in short day views (view: ${views[0].type}, offset: ${offset}, start: ${startDayHour}, end: ${endDayHour})`, async ({ page }) => {
        await insertStylesheetRulesToPage(page, REDUCE_CELLS_CSS);
        await createWidget(page, 'dxScheduler', {
        ...
        await testScreenshot(
          page,
          getScreenshotNameForEdgeCase('short-day-views', views[0].type, offset, startDayHour, endDayHour),
          { element: workSpace },
        );
      });
    });
