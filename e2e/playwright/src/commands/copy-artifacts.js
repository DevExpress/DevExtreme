...
const containerHtmlPath = path.resolve(__dirname, '../container.html');
...
assets.forEach((relPath) => {
    switch(true) {
        ...
        default:
            ...
            const destPath = path.join(ARTIFACTS_DIR, path.basename(relPath));
            fs.copyFileSync(srcPath, destPath);
            newHtml = newHtml.replace(new RegExp(relPath, 'g'), path.basename(relPath));
    }
});

const SOURCE_DIR = path.resolve(__dirname, '../../../packages/devextreme/artifacts');
