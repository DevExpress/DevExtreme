...
testOptions.forEach(({
  ...
  test(`targetedAppointmentData should be correct with groups (viewType="${viewType}", groupOrientation="${groupOrientation}", scrollMode="${scrollMode}", rtlEnabled="${rtlEnabled}") (T1205120)`, async ({ page }) => {
    // TestCafe failed a test on an uncaught page error; Playwright does not, so the errors the
    // template throws are collected and checked instead.
    ...
    expect(pageErrors, 'the appointment template reported a targetedAppointmentData mismatch').toEqual([]);
  });
});
