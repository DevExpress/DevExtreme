...
const getViewWithCorrectCellDuration = (
  view: { type: string; cellDuration?: number; firstDayOfWeek?: number },
  startDayHour: number,
  endDayHour: number,
): { type: string; cellDuration?: number; firstDayOfWeek?: number } => {
  switch (view.type) {
    case 'timelineWeek':
...
[
  ...
  [
    ...
    [
      ...
      test(`Recurrence appointments and workspaces render (view: ${views[0].type}, offset: ${offset}, start: ${startDayHour}, end: ${endDayHour}, first day: ${views[0].firstDayOfWeek})`, async ({ page }) => {
...
[
  ...
  [
    ...
    test(`Recurrence appointments and workspaces render in short day views (view: ${views[0].type}, offset: ${offset}, start: ${startDayHour}, end: ${endDayHour})`, async ({ page }) => {
      await insertStylesheetRulesToPage(page, REDUCE_CELLS_CSS);
      await createWidget(page, 'dxScheduler', {
