...
# on a developer machine that many browsers starve each other and the timing-sensitive tests fail
# for no reason. Half the cores unless the caller says otherwise — to reproduce a specific job,
# pass its matrix concurrency, e.g. --workers=3.
WORKERS=--workers=50%
for arg in "$@"; do
    case "$arg" in
