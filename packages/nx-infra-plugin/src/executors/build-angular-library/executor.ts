...
const ERROR_MESSAGES = {
  MISSING_NG_PACKAGE: (path: string) => `ng-package.json file not found at: ${path}`,
  MISSING_TSCONFIG: (path: string) => `TypeScript config file not found at: ${path}`,
...
const runExecutor: PromiseExecutor<BuildAngularLibraryExecutorSchema> = async (
  options,
  context,
) => {
  try {
    ...
    logError(ERROR_MESSAGES.BUILD_FAILED(context.projectName || 'unknown'), error);
