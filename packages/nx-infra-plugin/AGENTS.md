...
| `transpile-systemjs`         | `build:systemjs`                        | Uses `devextreme-nx-infra-plugin:transpile-systemjs` to spawn `node testing/systemjs-builder.js --transpile=<mode>` for each mode (`modules`, `testing`, `css`, `js-vendors`) in parallel — mirroring the old `gulp.parallel`. The heavy transform logic stays in `testing/systemjs-builder.js` (untouched, guaranteeing byte parity); the executor only orchestrates. `cache: true`, keeps the existing `inputs`/`outputs`/`dependsOn: [build:dev]`. Not a `dev-watch` member, so `build/gulp/systemjs.js`, its `gulpfile.js` require, and the `build:systemjs` npm script were deleted outright (no delegate). CI runs `pnpm exec nx build:systemjs` directly. |
