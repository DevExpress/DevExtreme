import { EditorValidationAdapter } from '@devextreme/interim';
...
interface LegacyEditorConnectorProps {
  ...
  setEditorAdapter: (adapter: EditorValidationAdapter) => void
}
...
export function LegacyEditorConnector({ validator, setEditorAdapter }: LegacyEditorConnectorProps) {
  ...
  useEffect(() => {
    ...
    const editorAdapter = new EditorValidationAdapter(legacyEditor, validator);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
