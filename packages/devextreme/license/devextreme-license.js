...
function renderFile(lcpKey, licenseId) {
    const lines = [
        '// Auto-generated by devextreme-license.',
        '// Do not commit this file to source control.',
    ];
    if(licenseId) {
        lines.push(`// License ID: ${licenseId}`);
    }
    lines.push('', `export const ${EXPORT_NAME} = ${JSON.stringify(lcpKey)};`, '');
    return lines.join('\n');
}
...
function renderNonModularFile(lcpKey, licenseId) {
    ...
    lines.push('', `DevExpress.config({ licenseKey: ${JSON.stringify(lcpKey)} });`, '');
    return lines.join('\n');
}
...
function main() {
    ...
    writeFileAtomic(outAbs, useNonModular ? renderNonModularFile(lcp, licenseId) : renderFile(lcp, licenseId));
