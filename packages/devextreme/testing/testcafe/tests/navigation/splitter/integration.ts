...
safeSizeTest('The splitter pane should be rendered with the correct ratio inside the tab content of TabPanel if pane.size uses pixels', async (t) => {
  ...
  await testScreenshot(t, takeScreenshot, 'Splitter inside tabPanel tab content, pane_1.size=`100px`.png', { element: '#container' });

  await t.resizeWindow(600, 400);

  await testScreenshot(t, takeScreenshot, 'Splitter inside tabPanel tab content after window resize, pane_1.size=`100px`.png', { element: '#container' });
