      ...
      ['outlined', 'underlined', 'filled'].forEach((editorStylingMode) => {
        [true, false].forEach((labelVisible) => {
          ['center', 'left', 'right'].forEach((labelAlignment) => {
            ...
            const testName = `Form,lMode=${formLabelMode},lLoc=${formLabelLocation},lVis=${labelVisible},lAl=${labelAlignment},e.lMode=${editorLabelMode ? 'undef' : editorLabelMode},e.sMode=${editorStylingMode}`;
