...
test('Accessibility: DataGrid total summary status should contains correct text after cell value editing', async (t) => {
  const expectedStatusText = 'Value Sum: 3000';
  const dataGrid = new DataGrid('#container');

  const cell = dataGrid.getDataCell(0, 1);
  await t.click(cell.element);
  const editor = cell.getEditor();
  await t.typeText(editor.element, '1050', { replace: true })
    .pressKey('enter');

  const statusText = await dataGrid.getSummaryCellStatusContainer(0).textContent;
