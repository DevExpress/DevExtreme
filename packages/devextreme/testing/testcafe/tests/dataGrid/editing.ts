...
test('Tab key on editor should not raise error when onSaving\'s promise is not yet resolved (T1190566)', async (t) => {
  ...
  const noErrorExists = !consoleMessages?.error.length;
  await t.expect(noErrorExists).ok();
