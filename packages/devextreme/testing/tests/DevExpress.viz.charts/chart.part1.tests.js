...
QUnit.module('dxChart', environment);
...
QUnit.test('Clear hover after series updating', function(assert) {
    ...
    getTrackerStub().stub('clearHover').reset();

    chart.option(options);

    assert.equal(getTrackerStub().stub('clearHover').callCount, 1);
});
...
QUnit.test('Refresh chart when series hovered', function(assert) {
    ...
    assert.equal(getTrackerStub().stub('clearHover').callCount, 0);
});
...
QUnit.test('Create Tracker.', function(assert) {
    ...
    assert.ok(getTrackerStub().stub('update').calledOnce, 'update was call once');
    const updateArg0 = getTrackerStub().stub('update').lastCall.args[0];
    ...
    assert.ok(getTrackerStub().stub('setCanvases').calledOnce, 'setCanvases for tracker');
    assert.deepEqual(getTrackerStub().stub('setCanvases').lastCall.args, [{
        bottom: 800,
        left: 0,
        right: 800,
        top: 0
    ...
    assert.ok(getTrackerStub().stub('updateSeries').calledOnce, 'updateSeries');
    assert.deepEqual(getTrackerStub().stub('updateSeries').lastCall.args[0], chart.series, 'updateSeries args');
});
...
QUnit.test('actions sequence on render chart', function(assert) {
    ...
    assert.ok(updatePanesCanvasesSpy.lastCall.calledAfter(getTitleStub().move.lastCall), 'second call updatePanes after draw title');
});
...
QUnit.module('LoadingIndicator', $.extend({}, environment, {
    beforeEach: function() {
...
QUnit.test('Schedule loading indicator hiding on data source changed event', function(assert) {
    // DataSource is already imported at the top of the file
...
QUnit.module('dxChart user options of strips', environment);
...
QUnit.module('dxChart user options of constant lines', environment);
...
QUnit.module('Render Complete callback', $.extend({}, environment, {
    beforeEach: function() {
...
// T370892
QUnit.module('passing data to series', $.extend({}, environment, {
