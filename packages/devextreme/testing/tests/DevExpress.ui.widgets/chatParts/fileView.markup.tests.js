...
import FileView, {
    FILE_VIEW_CLASS,
    FILE_VIEW_CONTAINER_CLASS,
    FILE_VIEW_ITEM_CLASS,
    CHAT_FILE_CLASS
...
QUnit.module('FileView', {
    ...
    test('should correct structure when no files passed', function(assert) {
        const $container = this.$element.find(`.${FILE_VIEW_CONTAINER_CLASS}`);

        assert.ok(this.$element.hasClass(FILE_VIEW_CLASS), 'root has correct class');
    ...
    test('should correct structure when files passed', function(assert) {
        const files = [{ name: 'Test', size: 50 }];
        this.reinit({ files });

        const $container = this.$element.find(`.${FILE_VIEW_CONTAINER_CLASS}`);
        const $fileViewItem = $container.find(`.${FILE_VIEW_ITEM_CLASS}`);
