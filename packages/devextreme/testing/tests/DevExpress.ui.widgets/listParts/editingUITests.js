...
QUnit.module('reordering decorator', {
    ...
    QUnit.test('list item duplicate should inherit direction in rtl (T859557)', function(assert) {
        const $list = $('#templated-list').dxList({
            items: ['0'],
            itemDragging: { allowReordering: true },
            rtlEnabled: true
    ...
    QUnit.test('Drag and drop item to the top of the list should not work when allowReordering is false (T856292)', function(assert) {
        const $list = $('#list').dxList({
            items: ['0', '1', '2'],
            itemDragging: {
