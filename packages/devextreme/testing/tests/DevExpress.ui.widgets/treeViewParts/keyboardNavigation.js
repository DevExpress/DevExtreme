...
QUnit.module('keyboard navigation', {
    ...
    QUnit.test('right arrow focus child node, if parent container is expanded', function(assert) {
        const data = $.extend(true, [], DATA[1]);
        data[0].expanded = false;
    ...
    QUnit.test('treeview should not lose focus when parent item is disabled (T303800)', function(assert) {
        ...
        const items = [{
            id: 1,
            text: 'Item 1',
            items: [{ id: 2, text: 'Item 2', disabled: true, expanded: true, items: [{ id: 21, text: 'Item 21' }] }]
        }, { id: 3, text: 'item 3' }];
        const $treeView = initTree({
            focusStateEnabled: true,
            items: items
    ...
    QUnit.module('keyboard navigation when showCheckBoxesMode=none and selectByClick=true', {
        ...
        ['single', 'multiple'].forEach((selectionMode) => {
            QUnit.test(`shift+home key press should select all items up to the beginning of the list, selectionMode=${selectionMode}`, function(assert) {
                const $treeView = this.createTreeView(selectionMode);
                const $item = $treeView.find(`.${internals.ITEM_CLASS}`).eq(-1);
            ...
            QUnit.test(`shift+end key press should select all items down to the end of the list, selectionMode=${selectionMode}`, function(assert) {
                const $treeView = this.createTreeView(selectionMode);
                const $item = $treeView.find(`.${internals.ITEM_CLASS}`).eq(0);
        ...
        QUnit.test('shift+down key press should select next item', function(assert) {
        ...
        QUnit.test('shift+up key press should select previous item', function(assert) {
            const $treeView = this.createTreeView();
            const $item = $treeView.find(`.${internals.ITEM_CLASS}`).eq(0);
