    ...
    const $etalon = $(
        '<div id="container" class="dx-filterbuilder dx-widget">'
            + '<div class="dx-filterbuilder-group" role="tree">'
                ...
                + '</div>'
                + '<div class="dx-filterbuilder-group-content" id="' + groupId + '" role="group"></div>'
            + '</div>'
        + '</div>'
    );
...
QUnit.test('filter Content init by one condition', function(assert) {
    ...
    const classLevel = 'dx-filterbuilder-group-item[aria-level="2"]';
    ...
    const fieldId = element.find(`.${classLevel} .${CLASSES.FILTER_BUILDER_ITEM_FIELD_CLASS}`).attr('aria-controls');
