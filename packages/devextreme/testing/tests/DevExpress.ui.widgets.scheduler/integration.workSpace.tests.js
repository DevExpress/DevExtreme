...
module(
    ...
    () => {
                ...
                resourceCellTemplate: function(cellData, cellIndex, cellElement) {
                    if(!cellIndex) {
                        ...
                        assert.roughEqual(
                            ...
                            299,
                            2.001,
                            'Resource cell width is OK',
                        );
                        assert.equal(
                            getOuterHeight($cell),
                            30,
                            'Resource cell height is OK',
                        );
                    }
                },
            });
        ...
        test('resourceCellTemplate should work correct in timeline view', async function(assert) {
            ...
            assert.ok(
                $cell1.hasClass('custom-group-cell-class'),
                'first cell has right class',
            );
            assert.notOk(
                $cell2.hasClass('custom-group-cell-class'),
                'second cell has no class',
            );
        });
        ...
        test('resourceCellTemplate should work correct in agenda view', async function(assert) {
            ...
            const $cell1 = scheduler.instance
                .$element()
                .find('.dx-scheduler-group-header-content')
                .eq(0);
            const $cell2 = scheduler.instance
                .$element()
                .find('.dx-scheduler-group-header-content')
                .eq(1);
