...
export function isContinuousIntegration(): boolean {
  const ccNetWorkingDirectory = process.env.CCNetWorkingDirectory;

  if (typeof ccNetWorkingDirectory === 'string' && ccNetWorkingDirectory.length > 0) {
    return true;
  }

  return Boolean(process.env.DEVEXTREME_TEST_CI);
}
