...
const timeIntervals = [
  {
    ...
    max: Date.UTC(2000, 0, 10, 10, 30),
  }, {
    min: Date.UTC(2000, 0, 11, 3, 30),
    max: Date.UTC(2000, 0, 11, 10, 30),
  }, {
    min: Date.UTC(2000, 0, 12, 3, 30),
    max: Date.UTC(2000, 0, 12, 10, 30),
  }, {
    min: Date.UTC(2000, 0, 13, 3, 30),
    max: Date.UTC(2000, 0, 13, 10, 30),
  }, {
    min: Date.UTC(2000, 0, 14, 3, 30),
    max: Date.UTC(2000, 0, 14, 10, 30),
  }, {
    min: Date.UTC(2000, 0, 15, 3, 30),
    max: Date.UTC(2000, 0, 15, 10, 30),
  },
];
...
describe('isAppointmentMatchedIntervals', () => {
  ...
  describe.each([
    ...
    it('should compare appointment intersect interval by end date', () => {
      expect(isAppointmentMatchedIntervals({
        ...
        endDate: Date.UTC(2000, 0, 10, 5),
      }, intervals)).toBe(true);
    });

    it('should compare appointment intersect interval by end date (hours)', () => {
      expect(isAppointmentMatchedIntervals({
        startDate: Date.UTC(2000, 0, 9, 5),
        endDate: Date.UTC(2000, 0, 10, 3, 0),
    ...
    it('should compare appointment inside interval', () => {
      expect(isAppointmentMatchedIntervals({
        startDate: Date.UTC(2000, 0, 12, 5),
        endDate: Date.UTC(2000, 0, 12, 8),
      }, intervals)).toBe(true);
    });

    it('should compare appointment inside gap between days', () => {
      expect(isAppointmentMatchedIntervals({
        startDate: Date.UTC(2000, 0, 12, 23),
        endDate: Date.UTC(2000, 0, 13, 2),
      }, intervals)).toBe(isDateOnly);
    });

    it('should compare appointment inside gap before interval same day', () => {
      expect(isAppointmentMatchedIntervals({
        startDate: Date.UTC(2000, 0, 12, 1),
        endDate: Date.UTC(2000, 0, 12, 2),
      }, intervals)).toBe(isDateOnly);
    });

    it('should compare appointment inside gap after interval same day', () => {
      expect(isAppointmentMatchedIntervals({
        startDate: Date.UTC(2000, 0, 12, 11),
        endDate: Date.UTC(2000, 0, 12, 12),
      }, intervals)).toBe(isDateOnly);
    });
  });
  ...
  it('should compare appointment hugging view interval', () => {
    expect(isAppointmentMatchedIntervals({
      startDate: Date.UTC(2000, 0, 9, 12),
      endDate: Date.UTC(2000, 0, 17, 10),
    }, viewIntervals)).toBe(true);
  });
