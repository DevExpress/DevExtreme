...
/**
 * A DST transition that happens inside one Scheduler-zone day.
 *
 * `deltaMs` is the offset after the jump minus the offset before it: negative for a
 * fall-back, where the wall clock repeats `-deltaMs` of time, positive for a
 * spring-forward, where `deltaMs` of wall clock never happens.
 */
export interface DaylightTransition {
  instant: Date;
  deltaMs: number;
  /** Wall clock the day shows right before the jump, in minutes from its midnight. */
  wallBeforeMinutes: number;
  /** UTC instant of wall 00:00 on that day. */
  anchorUTC: number;
}

/** One cell of the date table: an interval of elapsed time and the wall clock it shows. */
...
export interface DaylightPlanDay {
  ...
  anchorUTC: number;
  /** Wall clock of the day's first cell, as a wall-clock-in-UTC timestamp. */
  wallStartMs: number;
...
/** The Scheduler-zone wall clock of an instant, as a Date in the client zone. */
...
const detectTransition = (
  ...
  for (let time = scanStart + SCAN_STEP_MS; time <= scanEnd; time += SCAN_STEP_MS) {
    ...
    if (offset !== previousOffset) {
      ...
      const deltaMs = offset - previousOffset;
      const wallAfter = toGrid(instantMs, calculator);

      // The day that shows the jump owns it: for a midnight fall-back that is the
      // previous day, which then ends at wall 24:00 instead of 23:00.
 ...
 *
 * Wall clock and elapsed time run together up to the jump and are `deltaMs` apart
 * after it, so a repeated hour yields two sets of cells and a skipped one none.
 */
export const buildDayCells = (
  ...
  const transition = findTransition(day, calculator);
...
/**
 * The grid of one view, day by day. Returns undefined unless a day carries a DST
 * transition — every other view keeps the plain wall-clock layout.
 */
...
/**
 * Whether `sourceUTC` falls in the hour a fall-back plays a second time. Any other
 * instant, such as the start of a recurrent series, leaves the wall clock on its
 * first pass.
 */
 ...
 * from, used only to tell the two passes over a repeated hour apart.
 */
...
/**
 * Column of a wall clock, counting every cell as one column even when a clipped cell
 * is shorter than the configured duration.
 */
