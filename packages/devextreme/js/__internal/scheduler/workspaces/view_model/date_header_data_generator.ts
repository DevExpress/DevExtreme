...
const keepActualCellDate = (cellIndex: number, date: Date): Date => (
  cellIndex < 0 ? date : date
);
...
export class DateHeaderDataGenerator {
  ...
  private generateWeekDaysHeaderRowMap(
    ...
    const extraCellCounts = this.viewDataGenerator.getRepeatedHourExtraCellCounts(options);
    ...
    const hasRepeatedHour = extraCellCounts.some((count) => count > 0);
    ...
    for (let dayIndex = 0; dayIndex < daysInView; dayIndex += 1) {
      const extraCells = extraCellCounts[dayIndex % daysInGroup] ?? 0;
      const dayColSpan = hasRepeatedHour
        ? (isGroupedByDate ? horizontalGroupCount : 1) * (cellCountInDay + extraCells)
        : colSpan;
  ...
  private generateHeaderDateRow(
    ...
    const hasRepeatedHour = this.viewDataGenerator
      .getRepeatedHourExtraCellCounts(options)
      .some((count) => count > 0);
    const cellCountInDay = this.viewDataGenerator
      .getCellCountInDay(startDayHour, endDayHour, hoursInterval);
    ...
    return slicedByColumnsData.map((cellData, idx: number) => {
      ...
      const text = getHeaderCellText(
        ...
        headerCellTextFormat,
        hasRepeatedHour ? keepActualCellDate : getDateForHeaderText,
        {
          interval,
          startViewDate,
          startDayHour,
  ...
  private generateVariableSpanHeaderRow(
    ...
    return {
      ...
      rightVirtualCellCount: Math.max(0, totalCellCount - rightCovered),
      rightVirtualCellWidth: isProvideVirtualCellsWidth
        ? Math.max(0, totalCellCount - rightCovered) * cellWidth
        : undefined,
    };
  }
}
