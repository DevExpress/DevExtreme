...
class Scheduler extends SchedulerOptionsBaseWidget {
  ...
  private compactAppointmentsHelper!: CompactAppointmentsHelper;
  ...
  private layoutManager!: AppointmentLayoutManager;
  ...
  private postponeDataSourceLoading(promise?: any) {
    this.postponedOperations.add('reloadDataSource', this.reloadDataSource.bind(this), promise);
  }
  ...
  _optionChanged(args) {
    ...
    switch (args.name) {
      ...
      case 'currentDate': {
        ...
        this.appointments.option('items', []);
      ...
      case 'currentView':
        ...
        this.postponeResourceLoading().done(() => {
          ...
          this.setRemoteFilterIfNeeded();
          this.appointments.option('allowAllDayResize', value !== 'day');
        });
        // NOTE:
      ...
      case 'appointmentTemplate':
        this.appointments.option('itemTemplate', value);
        break;
      ...
      case 'onAppointmentRendered':
        this.appointments.option('onItemRendered', this.getAppointmentRenderedAction());
        break;
      case 'onAppointmentClick':
        this.appointments.option('onItemClick', this._createActionByOption(name));
        break;
      case 'onAppointmentDblClick':
        this.appointments.option(name, this._createActionByOption(name));
        break;
      case 'onAppointmentContextMenu':
        this.appointments.option('onItemContextMenu', this._createActionByOption(name));
      ...
      case 'crossScrollingEnabled':
        this.postponeResourceLoading().done(() => {
          ...
          if (this.readyToRenderAppointments) {
            this.appointments.option('items', this.layoutManager.generateViewModel());
          }
        });
        break;
      case 'cellDuration':
      ...
      case 'focusStateEnabled':
        this.updateOption('header', name, value);
        this.updateOption('workSpace', name, value);
        this.appointments.option(name, value);
        // @ts-expect-error
  ...
  private bringEditingModeToAppointments(editing) {
    ...
    this.appointments.option(editingConfig);
    this.repaint();
  }
  ...
  private isAllDayExpanded() {
    return this.option('showAllDayPanel') && this.layoutManager.hasAllDayAppointments();
  }
  ...
  private setRemoteFilterIfNeeded(): void {
    ...
    if (!this.workSpace || !remoteFiltering || !dataSource) {
      return;
    }
  ...
  _dimensionChanged(value, isForce = false) {
    ...
    if (
      ...
      if (isForce || (!isFixedHeight || !isFixedWidth)) {
        ...
        const appointments = this.layoutManager.generateViewModel();

        this.appointments.option('items', appointments);
      }
    }
  ...
  _init() {
    ...
    this.compactAppointmentsHelper = new CompactAppointmentsHelper(this);
  ...
  private renderAppointments() {
    const workspace = this.getWorkSpace();
    this.layoutManager.filterAppointments();

    workspace.option('allDayExpanded', this.isAllDayExpanded());

    // @ts-expect-error
    const viewModel: AppointmentViewModelPlain[] = this._isVisible()
      ? this.layoutManager.generateViewModel()
      : [];

    this.appointments.option('items', viewModel);
    this.appointmentDataSource.cleanState();
    if (this.isAgenda()) {
      this.workSpace.renderAgendaLayout(viewModel);
    }
  }

  private initExpressions(fields: IFieldExpr) {
    this.dataAccessors = new AppointmentDataAccessor(
      fields,
  ...
  private updateExpression(name: string, value: string) {
    this.dataAccessors.updateExpression(name, value);
  }
  ...
  private updateA11yStatus() {
    ...
    const label = getA11yStatusText(
      ...
      dateRange[0],
      dateRange[1],
      this.layoutManager.filteredItems.length,
      indicatorTime,
    );
  ...
  _initMarkup(): void {
    ...
    this.layoutManager = new AppointmentLayoutManager(this);

    // @ts-expect-error
    this.appointments = this._createComponent('<div>', AppointmentCollection, this.appointmentsConfig());
    this.appointments.option('itemTemplate', this.getAppointmentTemplate('appointmentTemplate'));
  ...
  createAppointmentForm() {
    const scheduler = {
      ...
      getCalculatedEndDate: (startDateWithStartHour) => this.workSpace.calculateEndDate(startDateWithStartHour),
  ...
  createAppointmentPopup(form) {
    const scheduler = {
      ...
      updateScrollPosition: (startDate, appointmentGroupValues, inAllDayRow) => {
        this.workSpace.updateScrollPosition(startDate, appointmentGroupValues, inAllDayRow);
      },
    };
  ...
  private getAppointmentTooltipOptions() {
    ...
    return {
      ...
      getAppointmentDisabled: (appointment) => this.dataAccessors.get('disabled', appointment),
      onItemContextMenu: that._createActionByOption('onAppointmentContextMenu'),
      createEventArgs: that.createEventArgs.bind(that),
    };
  }

  private createEventArgs(e) {
    const config = {
  ...
  private initMarkupCore() {
    ...
    this.workSpace && this.cleanWorkspace();
  ...
  private appointmentsConfig() {
    const config = {
      ...
      getResizableStep: () => (this.workSpace ? this.workSpace.positionHelper.getResizableStep() : 0),
      getDOMElementsMetaData: () => this.workSpace?.getDOMElementsMetaData(),
      getViewDataProvider: () => this.workSpace?.viewDataProvider,
      isVerticalGroupedWorkSpace: () => this.workSpace._isVerticalGroupedWorkSpace(),
      isDateAndTimeView: () => isDateAndTimeView(this.workSpace.type),
      onContentReady: () => {
        this.workSpace?.option('allDayExpanded', this.isAllDayExpanded());
      },
    };
  ...
  private renderWorkSpace() {
    ...
    if (currentViewOptions.startDate) {
      this.updateOption('header', 'currentDate', this.workSpace._getHeaderDate());
    }
  }
  ...
  renderGridWorkspace(): void {
    if (this.readyToRenderAppointments) {
      ...
      Promise.resolve().then(() => {
        this.toggleSmallClass();
        this.workSpace?.updateHeaderEmptyCellWidth();
      });
    }
