...
export class AgendaAppointment extends Appointment {
  get coloredElement(): dxElementWrapper {
    return (this.$element() as unknown as dxElementWrapper).find(`.${APPOINTMENT_CONTENT_CLASSES.AGENDA_MARKER}`);
  }
  ...
  _renderResourceList(): void {
    ...
    void this.resourceProcessor
      ...
      .then((list) => {
        const parent = (this.$element() as unknown as dxElementWrapper)
          .find(`.${APPOINTMENT_CONTENT_CLASSES.APPOINTMENT_CONTENT_DETAILS}`);
        const container = $('<div>')
