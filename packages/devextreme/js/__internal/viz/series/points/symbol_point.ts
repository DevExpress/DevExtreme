...
export default {
  ...
  _createSymbolMarker(renderer, pointSettings) {
    let marker;
  ...
  _createMarker(renderer, group, image, settings) {
    const that = this;
  ...
  _getLabelConnector(pointPosition) {
  ...
  _getPositionFromLocation() {
    return { x: this.x, y: this.y };
  },

  _isPointInVisibleArea(visibleArea, graphicBBox) {
  ...
  _getLabelCoordOfPosition(label, position) {
    const that = this;
    const labelBBox = label.getBoundingRect();
    const graphicBBox = that._getGraphicBBox(label.pointPosition);
    const offset = LABEL_OFFSET;
  ...
  setPercentValue(absTotal, total, leftHoleTotal, rightHoleTotal) {
    const that = this;
