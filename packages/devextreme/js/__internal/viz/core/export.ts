...
function setPrint(imageSrc, options) {
  return function () {
    ...
    const removeFrame = () => {
      /// #DEBUG
      options.__test?.checkAssertions();
      /// #ENDDEBUG
      this.parentElement.removeChild(this);
      /// #DEBUG
      options.__test?.deferred.resolve(origImageSrc, window?.document?.body?.style);
      /// #ENDDEBUG
    };
...
export let combineMarkups = function (widgets, options = { }) {
  ...
  const rootElement = wrapItemsToElement(
    totalWidth,
    totalHeight,
    backgroundColor,
    {
