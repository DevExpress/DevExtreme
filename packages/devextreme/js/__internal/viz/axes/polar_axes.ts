...
import xyAxes from '@ts/viz/axes/xy_axes';
import {
  convertPolarToXY, convertXYToPolar,
  getCosAndSin, getDistance,
  getVizRangeObject, map as _map, normalizeAngle,
...
const circularAxes = {
  ...
  _getSpiderCategoryOption() { // TODO rename spider
  ...
  _getStripLabelCoords(from, to) {
    ...
    return { x, y, align: constants.center };
  },
  ...
  _getTickMarkPoints(coords, length, { shift = 0 }) {
    const center = this.getCenter();
    const corrections = {
      inside: -1,
      center: -0.5,
...
export const circularSpider = extend({}, circularAxes, {
  ...
  _updateAxisElementPosition() {
    this._axisElement.attr({
      points: _map(this.getSpiderTicks(), (tick) => ({ x: tick.coords.x, y: tick.coords.y })),
    });
  },
  ...
  getSpiderTicks() {
    ...
    that._spiderTicks.forEach((tick) => { tick.initCoords(); });
...
export const linear = {
  ...
  _getStripLabelCoords(from, to) {
    ...
    return { x: center.x, y, align: constants.center };
  },
