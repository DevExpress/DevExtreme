...
function generateAutoBreaks({ logarithmBase, type, maxAutoBreakCount }, series, { minVisible, maxVisible }) {
  ...
  const getRange = type === 'logarithmic'
    ? (min, max) => getLog(max / min, logarithmBase)
    : (min, max) => max - min;
...
export default {
  linear: {
    ...
    _getMaxLabelHeight(boxes, spacing) {
      return getMaxSide((box) => box.height, boxes) + spacing;
    },
    ...
    _drawDateMarker(date, options, range) {
      ...
      if (!options.withoutStick) {
        pathElement = that._renderer.path([options.x, options.y, options.x, options.y + markerOptions.separatorHeight], 'line')
          .attr({
            'stroke-width': markerOptions.width, stroke: markerOptions.color, 'stroke-opacity': markerOptions.opacity, sharp: 'h',
          })
    ...
    _getScaleBreaks(axisOptions, viewport, series, isArgumentAxis) {
      ...
      if (axisOptions.type !== 'discrete' && axisOptions.dataType === 'datetime' && axisOptions.workdaysOnly) {
        breaks = breaks.concat(generateDateBreaks(
          viewport.minVisible,
          viewport.maxVisible,
    ...
    positionIsBoundary(position) {
      return [TOP, LEFT, BOTTOM, RIGHT].includes(position);
    },
