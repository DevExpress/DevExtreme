...
export function smartFormatter(tick, options) {
  ...
  if (!isDefined(format) && options.type !== 'discrete' && tick && (options.logarithmBase === 10 || !isLogarithmic)) {
    if (options.dataType !== 'datetime' && isDefined(tickInterval)) {
      ...
      } else if (isExponential(tickInterval) && (stringTick.includes('.') || isExponential(tick))) {
        format = {
          type: EXPONENTIAL,
...
export function formatRange({
  startValue, endValue, tickInterval, argumentFormat, axisOptions: { dataType, type, logarithmBase },
}) {
  if (type === 'discrete') {
    return '';
  }
