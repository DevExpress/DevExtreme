...
function drawCellsContent(doc, customDrawCell, cellsArray, docStyles) {
  cellsArray.forEach((cell) => {
    ...
    const eventArg = {
      doc, rect, pdfCell, gridCell, cancel: false,
    };
...
function drawBorders(doc, rect, {
  borderWidth, borderColor, drawLeftBorder = true, drawRightBorder = true, drawTopBorder = true, drawBottomBorder = true,
}, docStyles) {
  if (!isDefined(rect)) {
...
export {
  addNewPage, drawCellsContent, drawCellsLines, drawGridLines, drawLine, drawRect, drawTextInRect, getDocumentStyles, roundToThreeDecimals, setDocumentStyles,
};
