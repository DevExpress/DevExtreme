...
function calculateRowHeight(doc, cells, columnWidths) {
  ...
  for (let cellIndex = 0; cellIndex < cells.length; cellIndex++) {
    ...
    const { font } = cells[cellIndex].pdfCell;
    const { wordWrapEnabled } = cells[cellIndex].pdfCell;
...
export {
  applyRtl, applyWordWrap, calculateRowHeight, calculateTargetRectWidth, calculateTextHeight, getPageHeight, getPageWidth, getTextDimensions, getTextLines, toPdfUnit,
};
