...
function convertToCellsArray(rows) {
  return [].concat.apply(
    [],
    rows.map((rowInfo) => rowInfo.cells
      .filter((cell) => !isDefined(cell.pdfCell.isMerged))
      .map((cellInfo) => ({ ...cellInfo.pdfCell._rect, sourceCellInfo: { ...cellInfo.pdfCell, gridCell: cellInfo.gridCell } }))),
  );
}
...
function splitByPages(doc, rowsInfo, options, onSeparateRectHorizontally, onSeparateRectVertically) {
  ...
  const verticallyPages = splitRectsByPages(
    convertToCellsArray(rowsInfo),
    options.margin.top,
    'y',
    'h',
  ...
  while (pageIndex < verticallyPages.length) {
    // @ts-expect-error
    const horizontallyPages = splitRectsByPages(
      verticallyPages[pageIndex],
      options.margin.left,
      'x',
      'w',
  ...
  return verticallyPages.map((rects) => rects.map((rect) => ({ ...rect.sourceCellInfo, _rect: rect })));
}
