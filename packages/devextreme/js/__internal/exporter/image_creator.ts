...
import { each as _each, map as _map } from '@js/core/utils/iterator';
import {
...
function parseUrl(urlString) {
  const matches = urlString?.match(/url\(.*#(.*?)["']?\)/i);
  return matches?.[1];
}

function setFontStyle(context, options) {
  const fontParams: any[] = [];
...
function asyncEach(array, callback, d = Deferred()) {
  let i = 0;
...
function setLineDash(context, options) {
  let matches = options['stroke-dasharray']?.match(/(\d+)/g);
...
function getCanvasFromSvg(markup, {
  width, height, backgroundColor, margin, svgToCanvas = convertSvgToCanvas,
}) {
