...
import { DECODE_MAP, JS_EPOCH_TICKS, TICKS_PER_MILLISECOND } from './const';
...
export const dateToTicks = (date: Date): bigint => {
  const ms = BigInt(date.getTime());
  return ms * TICKS_PER_MILLISECOND + JS_EPOCH_TICKS;
};

export const nowTicks = (): bigint => dateToTicks(new Date());

export const encodeString = (text: string, encode: (s: string) => string): string => (typeof encode === 'function' ? encode(text) : text);
