...
export const getSize = function (element, dimension, box) {
  ...
  if (padding || border) {
    const paddingAndBorder = (!padding ? coeff * getComponentThickness(element, dimension, 'padding', styles) : padding)
            + (!border ? coeff * getComponentThickness(element, dimension, 'border', styles) : border);
