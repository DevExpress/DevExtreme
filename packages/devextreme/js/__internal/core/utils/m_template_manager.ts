...
export const acquireTemplate = (templateSource, createTemplate, templates, isAsyncTemplate, skipTemplates, defaultTemplates) => {
  ...
  const acquiredIntegrationTemplate: any = acquireIntegrationTemplate(
    templateSource,
    templates,
    isAsyncTemplate,
    skipTemplates,
  );
  const defaultTemplate = defaultTemplates[templateSource];
  const newSourceTemplate = createTemplate(templateSource);

  return acquiredIntegrationTemplate || defaultTemplate || newSourceTemplate;
};
