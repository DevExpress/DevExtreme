...
class ObservableValueContainerManager implements StateManagementTypes.ValueContainerManager {
...
export const observableValueContainerManagerFactory:
StateManagementTypes.ValueContainerManagerConstructor = {
  ...
  create(
    ...
    if (!isObservableValueContainer(valueContainer)) {
      throw new Error('Invalid value container for ObservableValueContainerManager');
    }

    return new ObservableValueContainerManager(logger, stateSourceSign, valueContainer);
  },
};
