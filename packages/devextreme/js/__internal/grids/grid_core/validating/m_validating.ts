...
export class ValidatingController extends modules.Controller {
  ...
  _isValidationInProgress: any;

  _currentCellValidator: any;

  _disableApplyValidationResults: any;
  ...
  validateCell(validator) {
    ...
    if (stateRestored && validationResult.status === VALIDATION_STATUS.pending) {
      this.updateCellValidationResult(cellParams as any);
...
export const editingControllerValidationExtenderMixin = (Base: ModuleType<EditingController>) => class EditingControllerValidationExtender extends Base {
  ...
  _showErrorRow(change) {
    let $popupContent;
    const errorHandling = this.getController('errorHandling' as any);
...
const editorFactory = (Base: ModuleType<EditorFactory>) => class EditorFactoryValidatingExtender extends Base {
...
const data = (Base: ModuleType<DataController>) => class DataValidationExtender extends Base {
...
const rowsView = (Base: ModuleType<RowsView>) => class RowsViewValidatingExtender extends Base {
  ...
  _formItemPrepared(cellOptions, $container) {
    // @ts-expect-error
    super._formItemPrepared.apply(this, arguments as any);
    deferUpdate(() => {
...
export const validatingModule = {
  ...
  extenders: {
    controllers: {
      editing: editingControllerValidationExtenderMixin,
      editorFactory,
      data,
    },
    views: {
