...
describe('Unsupported properties', () => {
  ...
  describe('Sorting properties', () => {
    it('should have no sorting state in the header (first load)', async () => {
      ...
      expect(aiTestHeader.attr('aria-sort')).not.toEqual('ascending');
    });
    it('should have no sorting state in the header (dynamic update)', async () => {
      const { instance } = await createDataGrid({
        dataSource,
        showBorders: true,
        columns: [
