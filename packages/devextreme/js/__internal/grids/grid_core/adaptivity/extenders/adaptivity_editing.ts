...
export const adaptivityEditingExtender = (
  Base: ModuleType<EditingController>,
): ModuleType<EditingController> => class AdaptivityEditingExtender extends Base {
  ...
  protected _beforeCloseEditCellInBatchMode(rowIndices) {
    // @ts-expect-error
    const expandedKey = this._dataController.adaptiveExpandedKey;
