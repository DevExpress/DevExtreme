...
class CalendarRangeSelectionStrategy extends CalendarSelectionStrategy {
  ...
  _getDaysInRange(startDate, endDate) {
    ...
    const firstDateInViews = dateUtils.getFirstMonthDate(dateUtils.getMonthInterval(currentDate, isAdditionalViewDate ? -2 : -1));
    const lastDateInViews = dateUtils.getLastMonthDate(dateUtils.getMonthInterval(currentDate, isAdditionalViewDate ? 1 : viewsCount));
