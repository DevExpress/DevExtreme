...
class TreeViewBase extends HierarchicalCollectionWidget<TreeViewBaseProperties, TreeViewNode> {
  ...
  _renderCheckBox($node: dxElementWrapper, node: TreeViewNode | null): void {
    if (!this._isItemShowCheckbox(node)) {
      return;
    }
  ...
  _isItemShowCheckbox(node: TreeViewNode | null): boolean {
    if (!node) {
      return true;
    }

    const { allowShowCheckbox } = this.option();

    if (typeof allowShowCheckbox === 'boolean') {
      return allowShowCheckbox;
    }
