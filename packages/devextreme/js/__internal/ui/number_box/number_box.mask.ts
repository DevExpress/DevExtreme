...
class NumberBoxMask extends NumberBoxBase<NumberBoxMaskProperties> {
  ...
  _parse(text: string, format: Format): NumberBoxValue | undefined {
    const customParser = asFormatObject(this._getEffectiveFormatOption())?.parser;

    if (isFunction(customParser)) {
      const customParsedValue = customParser(text);

      return customParsedValue instanceof Date ? undefined : customParsedValue;
    }
  ...
  _tryParse(
    ...
    const maxPrecision = hasCustomParser
      ? undefined
      : this._getPrecisionLimits(this._getFormatForSign(editedText)).max;
  ...
  _isValueIncomplete(text: string): boolean {
    ...
    const precision = this._getPrecisionLimits(this._getFormatForSign(asPattern(format)));
