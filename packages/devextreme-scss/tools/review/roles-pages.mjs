...
const RU = {
  ...
  'checkbox-focused-fill': 'Наполовину закрыт 15.09, когда дизайнер дала токены пакета для <code>checkbox/color/checked</code>. Слотов там <b>три, а не один</b>: у <code>border</code>, <code>bg</code> и <code>icon</code> свои токены — а наша тема красила контур переменной <b>заливки</b>.<br><br><b>Контур — применено.</b> Figma и реализация Blazor согласны: <code>border-primary-shared*</code>. Это не вопрос вкуса, поэтому сделано — три новых имени, светлый <b>пиксель в пиксель</b> прежний (primary-100/120/140 ровно как было), тёмный лучше на каждой ступени: контур коробки против страницы <b>2.88 → 4.65</b> в покое, 3.69 → 5.79 при наведении, 1.39 → 1.98 под нажатием.<br><br><b>Что осталось — заливка.</b> Figma говорит <code>bg-primary*</code>, Blazor реализует <code>content-primary-shared*</code>. В светлом обе <b>совпадают</b>, в тёмном расходятся — и это ровно то, что решает вопрос: на заливке Figma метка даёт <b>3.36</b> в покое и <b>1.62</b> под нажатием, на заливке Blazor — <b>5.42</b> и <b>2.31</b>. Ни одна не берёт 3.0 в нажатии: ступень <code>active</code> не инвертирована для тёмного ни в одном семействе primary — тот же дефект пакета, что записан в П2 для danger.<br>Довод за вариант Blazor помимо чисел: наша <b>невалидная</b> тройка уже сидит на стеме <code>content-danger-shared</code>, так что чекбокс сейчас внутренне непоследователен — невалидная заливка на <code>content-*-shared</code>, отмеченная на <code>bg-*</code>. У Blazor все три оттенка (primary, danger, success) на <code>content-&lt;оттенок&gt;-shared</code>.<br><br><b>И третье, что решают токены.</b> Список состояний в пакете — rest / hovered / active / read-only / disabled, <b>состояния focused там нет</b>. Наша тема красит <code>.dx-state-focused</code> и <code>.dx-state-active</code> одной переменной, то есть клавиатурный пользователь <b>сидит</b> в значении, которое пакет отвёл под кратковременное нажатие. Кнопка их уже развела (<code>bg-focused</code> читает <code>bg-primary-hovered</code>). Разделение оставит 1.62 там, куда его поставил пакет — под пальцем на время клика, — а клавиатуру уведёт на 4.30.<br><br><b>Пересверено 15.09 по свежему Blazor - довод про стем был неверен.</b> Заливки отмеченного у них: primary = <code>content-primary-shared</code>, а success и danger = <code>bg-success-shared</code> и <code>bg-danger-shared</code>. Я прочитал переменные <b>неопределённого</b> состояния (там чёрточка, и она content) как заливки отмеченного. Значит primary выбивается из самого Blazor, наше семейство <code>bg</code> совпадает с их success и danger, а наша невалидная <b>метка</b> на <code>content-danger-shared</code> совпадает с их неопределённой меткой — непоследовательности не было.<br><b>Зато появился кандидат, который устраивает оба источника:</b> <code>bg-primary-shared*</code>. Всё семейство <code>*-shared</code> — одно значение через bg, border и content, поэтому числа ровно как у Blazor: метка <b>5.42</b> в покое и коробка <b>4.65</b> в тёмном против сегодняшних 3.36 и 2.88. При этом роль остаётся в семействе <code>bg</code>, которое называет Figma, и на стеме, который сам Blazor держит для success и danger. Светлый — <b>как сейчас</b>. Ступень нажатия всё равно не берёт порог (2.31 / 1.98): ни одно семейство primary не инвертирует <code>active</code> для тёмного, это снова дефект из П2.',
  ...
  'radio-invalid-vs-checkbox-invalid': 'Начиналось как «радио против чекбокса», а при доведении категории Г до конца оказалось шире: <b>четыре контрола красят одну анатомию</b> — кольцо мелкого контрола плюс его заливку — и каждый решает её по-своему.<br>Ни один из этих семи вариантов не про пиксели: всё семейство <code>*-danger-shared</code> разрешается в одно значение через bg, border и content в обоих режимах. Вопрос в том, на какой анатомии семейство сходится.<br><b>Что мешает просто разделить:</b> переменная состояния покоя у радио — параметр миксина <code>dx-radiobutton-states-mixin</code> в базе, поэтому разделение меняет сигнатуру базы и задевает generic и material.<br><b>Варианты:</b> ① принять форму чекбокса везде — верная анатомия, ценой параметра базы; ② оставить по одной переменной на контрол, но называть и брать роль <b>по кольцу</b> — оно рисуется всегда, а заливка тогда открыто заимствует border-роль; ③ оставить как есть и записать расхождение. Рекомендация: ② сейчас (в пределах темы и без сдвига значений), ① — когда базу будут открывать по другим поводам.',
  'radio-checked-fill-stem': 'Заливка чекбокса переехала на стем <code>bg-primary-shared*</code>, а точка радиокнопки — нет, и в тот же день открылись три группы понятий: <code>checked bg rest</code>, <code>hovered</code> и <code>active</code>, в каждой checkBox на стеме shared, а radioButton на плоском.<br><br><b>Это не новый ущерб.</b> Радиокнопка всегда читала плоское семейство, а оно <b>не знает про режим</b>: <code>bg-primary</code> это #0f6cbd и в светлом, и в тёмном. Точка против страницы даёт <b>2.88</b> в покое и <b>1.39</b> под нажатием при пороге 3.0 для залитой графики. Стем даёт <b>4.65</b> и 1.98, и светлый при этом не двигается ни на одной ступени — ровно как у чекбокса.<br><br><b>Варианты:</b> ① перевести <code>radio-button-checked-bg*</code> на <code>bg-primary-shared*</code> — тот же ход, та же цена, и два контрола перестают расходиться; ② оставить и записать расхождение, то есть в одной форме два родственных контрола отвечают на один вопрос по-разному. Рекомендация — ①.',
  'tier-bypassed-by-neighbours': 'Нашлось 15.09, когда разбирали, какой ролью красится чекбокс при наведении. Сам чекбокс двигает <b>только край</b>: <code>border-contrast</code> → <code>border-contrast-hovered</code>, заливка не трогается — та же форма, что у плитки, и это подтверждает ответ по Д6.<br>Но <b>список перекрашивает тот же чекбокс</b> при наведении на <b>элемент</b>, и делает это, импортируя переменную темы чекбокса напрямую (<code>list/_index.scss</code> читает <code>checkBoxColors.$check-box-icon-checked-bg-hovered</code>), а не через собственное имя. В переменной лежит <code>ds.$color-bg-primary-hovered</code>, поэтому в собранном правиле краска идёт из <code>var(--dxds-color-bg-primary-hovered)</code> — имени дизайн-системы, без имени компонентного тира посередине.<br><br><b>Следствие проверено, а не предположено.</b> Каскад на настоящем бандле: отмеченный чекбокс сам по себе разрешается в <code>var(--dx-check-box-icon-checked-bg-hovered)</code>, а тот же чекбокс внутри списка с декоратором выбора — в <code>var(--dxds-color-bg-primary-hovered)</code>, потому что у селектора списка шесть классов против трёх. То есть <b>переопределение публичного имени чекбокса меняет контрол везде, кроме списка</b> — там, где это проверят в последнюю очередь.<br><br><b>Насколько широко.</b> <b>230</b> цветовых деклараций в бандле красят прямо из <code>--dxds-</code> без имени тира: больше всего в <code>toolbar-menu-section</code> (40), dataGrid (20), treeList (16), popup-content (10), menu-item (5), list (4). Размеры и типографика делают так же ещё 510 раз, но это <b>намеренно</b> — тир цветовой, шкалы лежат в дизайн-системе. Цветовая половина намеренной не была.<br>Тринадцать файлов темы импортируют цветовой модуль соседа внутри <code>_index.scss</code> и красят им напрямую; тот же импорт внутри <code>_colors.scss</code> безвреден — там он становится значением <b>собственного</b> имени виджета.<br><br><b>Варианты:</b> ① завести имя тира на каждую такую краску, со значением от соседа — как уже сделано в <code>chat/_colors.scss</code>: самое широкое, и тир растёт; ② починить только те случаи, где заимствованная краска <b>сталкивается</b> с существующим публичным именем — это и есть доказанный баг: list и scheduler поверх checkBox и radioButton; ③ записать 230 как осознанное и сказать в доках, что цвет вне имени тира не переопределяется. Рекомендация: ② сейчас, ① волной до выпуска тира — после релиза каждое недостающее имя это кастомизация, которую нельзя написать.',
...
const SPECIMEN = {
  ...
  'Д16': () => '<p class="meta">Контур — применено 15.09, светлый не двинулся:</p>' + row([
    tile('контур был', box({ bg: 'color-bg-primary', bd: 'color-bg-primary', fg: 'color-content-inverted', mark: 'tick' }),
      `<br><code>bg-primary</code><br>к странице ${ratios('color-bg-primary', 'color-bg', 3)}`),
    tile('контур стал', box({ bg: 'color-bg-primary', bd: 'color-border-primary-shared', fg: 'color-content-inverted', mark: 'tick' }),
      `<br><code>border-primary-shared</code><br>к странице ${ratios('color-border-primary-shared', 'color-bg', 3)}`),
  ])
  + '<p class="meta">Заливка — здесь Figma и реализация Blazor расходятся, и в светлом это незаметно:</p>'
  + row([
    tile('Figma: покой', box({ bg: 'color-bg-primary', bd: 'color-border-primary-shared', fg: 'color-content-inverted', mark: 'tick' }),
      `<br><code>bg-primary</code><br>метка ${ratios('color-content-inverted', 'color-bg-primary', 3)}`),
    tile('Blazor: покой', box({ bg: 'color-content-primary-shared', bd: 'color-border-primary-shared', fg: 'color-content-inverted', mark: 'tick' }),
      `<br><code>content-primary-shared</code><br>метка ${ratios('color-content-inverted', 'color-content-primary-shared', 3)}`),
    tile('кандидат: bg-shared', box({ bg: 'color-bg-primary-shared', bd: 'color-border-primary-shared', fg: 'color-content-inverted', mark: 'tick' }),
      `<br><code>bg-primary-shared</code><br>метка ${ratios('color-content-inverted', 'color-bg-primary-shared', 3)}`),
    tile('Figma: нажатие', box({ bg: 'color-bg-primary-active', bd: 'color-border-primary-shared-active', fg: 'color-content-inverted', mark: 'tick' }),
      `<br><code>bg-primary-active</code><br>метка ${ratios('color-content-inverted', 'color-bg-primary-active', 3)}`),
    tile('Blazor: нажатие', box({ bg: 'color-content-primary-shared-active', bd: 'color-border-primary-shared-active', fg: 'color-content-inverted', mark: 'tick' }),
      `<br><code>content-primary-shared-active</code><br>метка ${ratios('color-content-inverted', 'color-content-primary-shared-active', 3)}`),
  ])
  + '<p class="meta">И состояния: у пакета нет focused, есть active. Мы красим оба одной переменной:</p>'
  + row([
  ...
  'Д24': () => '<p class="meta">Точка радиокнопки против страницы, порог 3.0:</p>' + row([
    tile('сейчас, покой', ring({ bd: 'color-border-contrast', fill: 'color-bg-primary' }),
      `<br><code>bg-primary</code><br>${ratios('color-bg-primary', 'color-bg', 3)}`),
    tile('стем, покой', ring({ bd: 'color-border-contrast', fill: 'color-bg-primary-shared' }),
      `<br><code>bg-primary-shared</code><br>${ratios('color-bg-primary-shared', 'color-bg', 3)}`),
    tile('сейчас, нажатие', ring({ bd: 'color-border-contrast', fill: 'color-bg-primary-active' }),
      `<br><code>bg-primary-active</code><br>${ratios('color-bg-primary-active', 'color-bg', 3)}`),
    tile('стем, нажатие', ring({ bd: 'color-border-contrast', fill: 'color-bg-primary-shared-active' }),
      `<br><code>bg-primary-shared-active</code><br>${ratios('color-bg-primary-shared-active', 'color-bg', 3)}`),
  ])
  + '<p class="meta">Для сравнения — чекбокс рядом, уже на стеме:</p>'
  + row([
    tile('чекбокс, покой', box({ bg: 'color-bg-primary-shared', bd: 'color-border-primary-shared', fg: 'color-content-inverted', mark: 'tick' }),
      `<br>метка ${ratios('color-content-inverted', 'color-bg-primary-shared', 3)}`),
  ]),

  'Д23': () => '<p class="meta">Отмеченный чекбокс при наведении - одна и та же краска, разные цепочки:</p>'
    + row([
      tile('чекбокс сам по себе', box({ bg: 'color-bg-primary-hovered', bd: 'color-bg-primary-hovered', fg: 'color-content-inverted', mark: 'tick' }),
        '<br>из <code>--dx-check-box-<br>icon-checked-bg-hovered</code>'),
      tile('он же внутри списка', box({ bg: 'color-bg-primary-hovered', bd: 'color-bg-primary-hovered', fg: 'color-content-inverted', mark: 'tick' }),
        '<br>из <code>--dxds-color-<br>bg-primary-hovered</code><br>имени тира нет'),
    ])
    + '<p class="meta">Цвет сегодня один - и потому это видно только при переопределении: '
    + 'подмена публичного имени чекбокса дойдёт до левого образца и не дойдёт до правого. '
    + 'Ниже - неотмеченный чекбокс, где сигнал даёт край, как у плитки:</p>'
    + row([
      tile('покой', box({ bg: 'color-content-inverted', bd: 'color-border-contrast' }),
        `<br><code>border-contrast</code><br>${ratios('color-border-contrast', 'color-content-inverted', 3)}`),
      tile('наведение', box({ bg: 'color-content-inverted', bd: 'color-border-contrast-hovered' }),
        `<br><code>border-contrast-hovered</code><br>${ratios('color-border-contrast-hovered', 'color-content-inverted', 3)}`),
      tile('нажатие', box({ bg: 'color-content-inverted', bd: 'color-border-contrast-active' }),
        `<br><code>border-contrast-active</code><br>${ratios('color-border-contrast-active', 'color-content-inverted', 3)}`),
    ]),
