...
const RU = {
  ...
  'list-border-disabled': '<b>Это не вопрос контраста — это мёртвое объявление.</b> Переменная красит <code>.dx-list-select-all-label::after</code>, и во всём бандле этот псевдоэлемент задевает ровно одно правило — без <code>content</code>. Без него псевдоэлемент не создаётся, и <code>color</code> не красит ничего. Собственный неактивный текст метки при этом корректен строкой выше, на <code>--dx-list-content-disabled</code>.<br><b>Варианты:</b> ① удалить правило и переменную — чище всего, но <code>--dx-list-border-disabled</code> публичное имя, нужен ченджлог; ② удалить только правило, имя останется среди 17 курируемых без читателя; ③ оставить и записать. Рекомендация — ①.',
...
<p>Роль верна, врёт слово в имени. Публичные имена <code>--dx-*</code>: тир эволюционирующий,
переименование допустимо, но с ченджлогом и отдельной волной.</p>
