...
const RU = {
  ...
  'tree-focus-on-disabled-node': 'Нашлось 15.09 <b>клавиатурой, а не чтением кода</b>: стрелки заводят фокус на неактивный узел - дерево его не пропускает, <code>aria-disabled</code> стоит на <code>li</code>, а класс <code>dx-state-focused</code> узел всё равно получает. В дереве <b>с чекбоксами</b> там не красится ничего. Фокусное правило намеренно исключает неактивный чекбокс (его disabled-краска должна побеждать), а базовый миксин красит фокусный фон только у <code>.dx-treeview-item-without-checkbox</code> - значит и у строки нет ничего.<br>Померено на собранном бандле: активный узел в фокусе - рамка чекбокса #005397 / #67a2e1; <b>неактивный узел в фокусе - #d7d7d7 / #4c4c4c, ровно то же, что и без фокуса</b>. Вариант <b>без чекбоксов</b> индикацию даёт: строка берёт <code>bg-hovered</code> #f5f5f5 / #3b3b3b, неактивная она или нет. То есть один виджет несёт два разных контракта фокуса, и в одном из них дыра. Это WCAG 2.4.7: индикатор фокуса исчезает посреди списка.<br><br><b>Роли под это у пакета нет и не должно быть.</b> Сочетания «фокус + неактивно» в наборе нет вообще: есть <code>global.color.focus</code>, <code>-inverted</code>, <code>-static</code>, <code>-static-inverted</code> и <code>global.border-width.focus</code> - и ни одной перекрёстной с состоянием. Потому что дизайн-система отмечает фокус <b>отдельным контуром</b>, а не перекраской контрола. Blazor ровно это и делает: <code>treeview/layout.scss</code> рисует <code>outline</code> на <code>.dxbl-treeview-item-container:focus-visible</code> цветом <code>ds.$color-border-focus</code>, и тот же файл <b>явно гасит</b> собственный фокусный контур чекбокса внутри дерева (<code>--dxbl-checkbox-focused-check-element-frame-outline-color: transparent</code>). generic, material и legacy fluent красят чекбокс без охраны <code>:not(disabled)</code> - индикация у них есть, ценой того, что неактивный контрол выглядит активным.<br><br><b>Варианты:</b> ① дать варианту с чекбоксами тот же фокусный фон строки, который уже есть у варианта без них (<code>--dx-tree-view-bg-focused</code>, <code>bg-hovered</code>) - одно правило, новой роли не нужно, дыра закрывается для любого узла, а покраска чекбокса остаётся дополнительным аффордансом для активных; ② перевести дерево на системный фокус-прямоугольник (<code>--dx-focus-rect-outline</code>, им уже пользуются toolbar, dropDownMenu и pivotGrid) - это в точности модель Blazor, но наш прямоугольник управляется классом, поэтому покажется и по клику мышью, тогда как те три виджета прячут его за <code>:focus-visible</code>; ③ снять охрану <code>:not(.dx-state-disabled)</code> и красить неактивный чекбокс, как делают три другие темы - дешевле всего, но охрану 09.09 завели именно потому, что так контрол врёт про своё состояние. Рекомендация: ① сейчас, ② - вместе со стоящим вопросом про фокус-кольцо, куда относятся и четыре неиспользуемых фокусных роли пакета.',
  'radio-focus-rung-and-family': 'Поднято 15.09 с работающего виджета: радиокнопка в фокусе выглядит затемнённой. Померили - так и есть. Кольцо и точка читают <code>--dx-radio-button-checked-bg-active</code> = <code>bg-primary-shared-active</code>: #003c70 в светлом, #005397 в тёмном. Чекбокс после Д16 читает ступень <b>hovered</b> - <code>border-primary-shared-hovered</code> для кольца и <code>bg-primary-shared-hovered</code> для заливки: #005397 и #67a2e1. Два контрола одного семейства, две разные ступени под одно и то же состояние.<br><br><b>Дефект - в тёмном.</b> Рампа primary не переворачивает ступень active: <code>bg-primary-shared</code> идёт #0f6cbd → #4b90d9 между режимами, hovered #005397 → #67a2e1, а active - только #003c70 → #005397. Поэтому в тёмном точка в фокусе это #005397 на поверхности #242424, то есть <b>1.98:1</b> при пороге 3:1 для нетекстового индикатора - и <b>хуже, чем та же радиокнопка в покое</b> (4.65) или при наведении (5.79). Фокус делает контрол менее заметным - ровно тот же отказ, что был у неотмеченного чекбокса в дереве до 15.09. В светлом уходит в другую сторону и просто очень тёмный: 11.17:1.<br><br><b>В том же блоке ещё две находки.</b> (а) Семейство: кольцо это граница, а красится она <code>bg</code>-ролью. Чекбоксу границу перевели на <code>border-*</code> в Д16, радиокнопке - нет. (б) Мёртвое объявление: <code>$radio-button-icon-bg-focused</code> (<code>bg-alpha-hovered</code>) уходит в базовый миксин как цвет ripple, а fluent-next ripple тут же гасит - у <code>.dx-radiobutton-icon::after</code> в одном правиле стоят и <code>content: ""</code>, и <code>content: none</code>, а размеры нулевые. Роль читается, гейт достижимости чтение видит, краски нет. Тем же вызовом в ripple-ветку уходят <code>icon-border-read-only</code>, hover и active - а ветка <code>@else</code>, которая бы их использовала, вообще не эмитится.<br><br><b>Blazor отвечает на весь блок так же, как на дерево:</b> фокус там это контур вокруг элемента-галочки - <code>checkbox-base.$focused-check-element-frame-outline-color</code> = <code>ds.$color-border-focus</code>, толщина <code>border-width-20</code>, отступ <code>border-width-10</code>, - а собственные цвета контрола в фокусе <b>не меняются вовсе</b>. Покой и наведение у него совпадают с нашими точно (<code>color-border-contrast</code> / <code>-hovered</code>), так что расходимся мы только фокусом.<br><br><b>Варианты:</b> ① радиокнопка берёт решения чекбокса - кольцо на <code>border-primary-shared-hovered</code>, точка на <code>bg-primary-shared-hovered</code>: чинит тёмный до 5.79, чинит семейство и сводит два контрола вместе; ② оставить ступень active и принять 1.98 в тёмном - это требует обоснования, потому что ту же ступень у чекбокса отклонили в Д16; ③ увести оба контрола на фокусный контур ролью <code>border-focus</code>, как у Blazor - честный ответ и ровно тот, который уже спрашивает стоящий вопрос про фокус-кольцо. Мёртвый <code>$radio-button-icon-bg-focused</code> уходит при любом из трёх. Рекомендация: ① сейчас, ③ - вместе с решением по фокус-кольцу.',
};
const ruText = (key) => {
  const text = RU[key];
...
const SPECIMEN = {
  ...
  'Д26': () => '<p class="meta">Радиокнопка по состояниям - фокус уходит на ступень <code>active</code>:</p>'
    + row([
      tile('радио, покой', ring({ bd: 'color-bg-primary-shared', fill: 'color-bg-primary-shared' }),
        `<br><code>bg-primary-shared</code><br>${ratios('color-bg-primary-shared', 'color-bg', 3)}`),
      tile('радио, наведение', ring({ bd: 'color-bg-primary-shared-hovered', fill: 'color-bg-primary-shared-hovered' }),
        `<br><code>bg-primary-shared-hovered</code><br>${ratios('color-bg-primary-shared-hovered', 'color-bg', 3)}`),
      tile('радио, фокус', ring({ bd: 'color-bg-primary-shared-active', fill: 'color-bg-primary-shared-active' }),
        `<br><code>bg-primary-shared-active</code><br>${ratios('color-bg-primary-shared-active', 'color-bg', 3)}`),
    ])
    + '<p class="meta">Чекбокс в том же состоянии после Д16 - ступень <code>hovered</code> и border-роль на границе:</p>'
    + row([
      tile('чекбокс, фокус', box({ bg: 'color-bg-primary-shared-hovered', bd: 'color-border-primary-shared-hovered', fg: 'color-content-inverted', mark: 'tick' }),
        `<br><code>bg/border-primary-shared-hovered</code><br>${ratios('color-bg-primary-shared-hovered', 'color-bg', 3)}`),
      tile('он же, покой', box({ bg: 'color-bg-primary-shared', bd: 'color-border-primary-shared', fg: 'color-content-inverted', mark: 'tick' }),
        '<br><code>bg/border-primary-shared</code>'),
    ]),
