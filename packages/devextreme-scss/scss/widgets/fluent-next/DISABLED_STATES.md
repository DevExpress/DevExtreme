...
- **`dataGrid` не имеет класса `dx-datagrid` на корне виджета** — в отличие от `treeList`,
  который несёт `dx-treelist` на корне; у грида класс висит на контейнере уровнем ниже
  (`gridView.render()`), а `dx-state-disabled` — на корне. Первая правка развела селектор на
  `.dx-#{$name}.dx-state-disabled` и `.dx-state-disabled > .dx-#{$name}`, и этого **не хватило**:
  покраска заработала, но дим остался, потому что `opacity: 1` на потомке не отменяет групповую
  непрозрачность родителя. Гашение дима вынесено отдельным правилом на корень, до которого у
  грида можно дотянуться только через `.dx-state-disabled:has(> .dx-#{$name})`. Комбинатор
  прямого потомка здесь несущий: без него правило сняло бы дим с любого выключенного контейнера,
  внутри которого где-то лежит грид. Проверено — у девяти компонентов, всё ещё живущих на диме
  (`fileManager` в их числе), дим на месте;
...
**Оба режима проверены.** Замер повторён на `fluent-next.blue.dark`: все переведённые
компоненты красятся `#ffffff` → `#767676` (`color-content-disabled` тёмного режима), список
держащихся на диме совпадает со светлым, «disabled неотличим от включённого» — ноль.
