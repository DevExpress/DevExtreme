 ...
 * Игнорирует файлы с суффиксами _etalon, _diff, _mask
 * Использование: node update-clean-etalons.js <source_folder> [target_folder]
 */

const sourceFolder = process.argv[2] || './new-screenshots';
const targetFolder = process.argv[3] || './apps/demos/testing/etalons';

console.log('🔄 Обновление эталонных скриншотов (только чистые файлы)');
...
// Получаем список PNG файлов, исключая служебные суффиксы
function getCleanPngFiles(dir) {
    const files = [];
    ...
    function walkDir(currentDir) {
        ...
        for (const item of items) {
            ...
            if (stat.isDirectory()) {
                ...
                // Исключаем файлы с суффиксами _etalon, _diff, _mask
                if (!item.includes('_etalon.') && !item.includes('_diff.') && !item.includes('_mask.')) {
                    files.push(fullPath);
                }
            }
        }
...
const sourceFiles = getCleanPngFiles(sourceFolder);
const targetFiles = getAllPngFiles(targetFolder);
...
console.log(`   Новых скриншотов (чистых): ${sourceFiles.length}`);
